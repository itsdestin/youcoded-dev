...
- **A four-game arcade** — **Chess and Connect Four** head-to-head against a real person over the network (board, moves, rematch, in-game chat), **Flappy Bird and 2048** solo with local bests and friend leaderboards, in a docked side panel beside the chat — **while the agent works** (the "social AI" pillar). **Status 2026-08-31: built on branch `feat/games-arcade-shell` and NOT merged to master** (master still has only `ConnectFourBoard.tsx`), so anything shipping today is Connect Four alone. Head-to-head reporting is built on the Worker but no client has sent one yet.
