...
- [ ] Mascot rig system: mix-and-match library + theme-builder generation `feature` `#themes` `#buddy` (added 2026-07-16)
  Visual direction settled in the 2026-07-16 buddy-rig-workbench prototype (claude.ai artifact 411658ae): six approved skins (2.5D soft, clay, comic pop, comic burst, newsprint, thin-border sticker), welcome-model curious face with sparkle tracking pupils, component slots (hat/eyewear/item). Authoring library SHIPPED to wecoded-themes `mascots/` (PR #12): skins + Kuromi/Strawberry-Kitty example rigs + drop-in components + from-scratch generation constraints. Fold-back DONE 2026-07-16: all workbench decisions (companions, motion styles + intensity, unified springs, corrected pose signs, tx/ty poses, the approved "75° wider" side-peek with rig-hand-peek-* grip mittens) are now in the buddy-floater spec §3/§5/§6 + plan Tasks 7–12 (incl. the Task 8 sign fix pinned by test), the prototype is committed at docs/active/prototypes/2026-07-16-buddy-rig-workbench.html, and mascots/README documents grip hands/companions/tail (wecoded-themes PR #13, merged 90efb05). Rig core SHIPPED 2026-07-16 (youcoded PR #150, plan Tasks 7–9): sanitizer, mascot-poses (corrected signs pinned by test), MascotRig renderer, 2.5D-soft default rig, `mascot.rig` theme key — nothing consumes MascotRig until the BuddyMascot integration (buddy-floater plan Tasks 10–12, needs the action-bar IPC). Remaining scope: (1) `/theme-builder` mascot phase — mix/match from the library folder OR full generation under the README constraints; (2) rebuild golden-sunbreak + halftone-dimension as rigs (their currentColor art renders black via the current <img> path — rig inlining fixes the tint).
