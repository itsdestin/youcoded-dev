...
- [ ] VM testing flow for first-run install/setup/sign-in (Linux host) `feature` `#tests` `#install` (added 2026-07-15)
  ...
  **Update 2026-07-16 — infrastructure DONE, first test pass is all that remains.** Both guests provisioned on quickemu 4.9.9 + qemu-desktop: Windows 11 25H2 (~15 min install at native KVM speed) and Ubuntu 24.04, each with a verified `clean` snapshot — revert→boot lands on the desktop in ~45–50 s on both. Recipe + six verified gotchas (qemu-base has no display backends; quickget's MS ISO download is IP-blocked; the t=3s press-any-key→PXE trap; SPICE/egl-headless renders black on the 8060S; language/keyboard prompts; monitor-socket traps) in `docs/vm-testing.md`; macOS deliberately ruled out there (license + AMD). Remaining: run the first real installer/first-run/sign-in pass against the Win11 baseline. `scripts/vm/` helpers only after that pass (2026-04-29 lesson: untested provisioning scripts fail silently).
