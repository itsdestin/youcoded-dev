...
- **Optimistic bubble and PTY send derive from ONE sanitized string** (`outgoing-message.ts`) — the transcript confirms by content, and the send swaps newlines and tabs (CC drops a typed tab) for spaces. Divergence leaves bubbles `pending` forever.
...
- **Dedup on unchanged cols/rows BEFORE the resize IPC.** Windows ConPTY reflows and re-emits its buffer on every resize, and ResizeObserver + `proposeDimensions()` fire spuriously → CC's Ink UI lands in xterm scrollback. Dedup is a closure (`lastCols`/`lastRows`) in the renderer-side mount effect.
