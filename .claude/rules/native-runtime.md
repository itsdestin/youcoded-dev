...
## M2 — conversations & sync participation (`conversations/service.ts`, `native-title-feeder.ts`, `model/ModelPicker.tsx`) — guards: `session-meta-parity.test.ts`, `native-title-feeder.test.ts`, `holder-takeover.test.ts` (native flows)
...
Branch guards: `skill-catalog.test.ts`, `skill-tool.test.ts`, `skill-tool-gating.test.ts`, `injection-budget.test.ts`, `project-instruction-budget.test.ts`, `path-triggers.test.ts`, `rule-injection.test.ts`, `slash-routing.test.ts`, `tool-registry-manifest.test.ts`, `ipc-channels.test.ts`. Depth: `youcoded/docs/native-runtime.md` → "Skills, rules and injection (M3)".
...
- **The ROOT project-instruction file obeys the SAME budget via `fitProjectInstructions` — never a raw `.slice()`** — it sits in the byte-stable prompt, not a message, so it has its own fitter, but shares `injection-budget.ts` (one chars-per-token constant) and `profile.injectionBudgetTokens`. It also names the file and counts dropped sections; cuts prefer heading → paragraph → line, 50% floor. **Sizing is fixed at session start — `setBinding` does NOT re-apply it** (the prompt is never reassembled; that freeze IS the KV-cache prefix). · why: was `.slice(0, 20_000)` until 2026-08-10 — chars not tokens, same for frontier and 8k, byte-offset, silent; dropping 4,682 chars of this workspace's own `CLAUDE.md` every native turn (§7 item 3). · Guard: `project-instruction-budget.test.ts`.
