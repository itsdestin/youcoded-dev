...
- **Whole-file replay was load-bearing for things that never asked for it.** Removing it broke THREE features that had silently come to depend on it as a side effect, none of which any of 6,600 tests noticed (2026-08-28): history was only requested from three call sites, so a session arriving by any other route rendered empty; the Files drawer's list was only refreshed because the artifact tool-use tracker sees replayed tool events; and the page re-rendered live entries because the scratch replay started with an empty `seenUuids`. **Before removing a broadcast, enumerate its listeners** — `rg` the emit channel, not the function name — and ask of each: what does this do when the broadcast stops? Every one of the three fixes was to make the consumer ask for what it needs rather than wait to be told.
