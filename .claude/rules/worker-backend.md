...
- **NEVER log raw `device_id_hash` server-side outside `count(DISTINCT)`** — the privacy-by-construction promise (`AboutPopup.tsx`) is that `blob2` appears only inside cardinality aggregates (`count(DISTINCT blob2)`, or `count()`/`countIf` over a per-device subquery that never selects `blob2` outward, as `/installs` and `/retention` do). One event type only; the retired `install` event returns 410 ("new installs/day" = `MIN(timestamp) GROUP BY blob2`).
...
- **CF Analytics Engine SQL is a narrow subset, NOT full ClickHouse** — `count(DISTINCT)` only, quoted `INTERVAL '30' DAY`, its own narrow-scope token, no JOIN/UNION/CTE/`CASE`/`dateDiff`; a subquery in FROM works in production (`/installs`), `countIf`, `if`, `toStartOfWeek`, `toUnixTimestamp`, `LIKE`, `HAVING` are documented. The four 422 gotchas in full: `docs/worker-backend.md`.
