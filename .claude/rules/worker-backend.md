...
`wecoded-marketplace/worker/` — Cloudflare Worker: install counts, ratings, **thumbs + comments** (ratings stay for old clients), accounts/social, sync-hub, privacy analytics. Auto-deploys on push to master (`worker-deploy.yml`); **never tell Destin to run `wrangler deploy` manually.** Backend + analytics depth: `wecoded-marketplace/docs/worker-backend.md`; deploy flow: workspace `docs/build-and-release.md → Worker`.
...
- **`checkRateLimit()` limits NOTHING today — measured** (160 requests vs a 60/min cap, zero 429s, 2026-08-28): the Cache API is a no-op on `*.workers.dev`; a D1 counter would charge a write per anonymous read; counters are per-colo.
...
Opt-outable anonymous device-hash + DAU/MAU. Design: `docs/archive/specs/2026-05-01-device-id-analytics-design.md` (not the older install_id spec).
...
- **CF Analytics Engine SQL is a narrow subset, NOT full ClickHouse** — `count(DISTINCT)` only, quoted `INTERVAL '30' DAY`, no JOIN/UNION/CTE; FROM-subqueries work; `toStartOfWeek` returns SUNDAYS despite its docs. The four 422 gotchas in full: `docs/worker-backend.md`.
