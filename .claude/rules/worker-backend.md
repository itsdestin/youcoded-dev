...
`wecoded-marketplace/worker/` — Cloudflare Worker: install counts, ratings, **thumbs + comments** (shipped 2026-08-28; ratings stay for old clients), accounts/social, sync-hub, privacy analytics. Auto-deploys on push to master (`worker-deploy.yml`); **never tell Destin to run `wrangler deploy` manually.** `worker-ci.yml` typechecks + tests `worker/**` PRs pre-merge. Depth: `wecoded-marketplace/docs/worker-backend.md`; deploy flow: `docs/build-and-release.md → Worker`.

## Deploy / config invariants
- **Never put a key in both `[vars]` and `wrangler secret put` with the same name.** On every deploy the wrangler.toml var wins and silently clobbers the secret (symptom: OAuth breaks on a placeholder `GH_CLIENT_ID`). Define shared secrets ONLY as secrets — the missing-var pattern is load-bearing; don't add a placeholder.
...
- **`checkRateLimit()` limits NOTHING today — measured** (zero 429s, 2026-08-28): the Cache API is a no-op on `*.workers.dev`. Fix = custom domain; a D1 counter would charge a write per anonymous read.
...
Opt-outable anonymous device-hash + DAU/MAU. Current design: `docs/archive/specs/2026-05-01-device-id-analytics-design.md` (read this, not the older install_id spec).
...
- **NEVER log raw `device_id_hash` server-side outside `count(DISTINCT)`** — the privacy-by-construction promise (`AboutPopup.tsx`) is that `blob2` appears only inside cardinality aggregates (incl. per-device subqueries, as `/retention`). One event type only; the retired `install` event returns 410 ("new installs/day" = `MIN(timestamp) GROUP BY blob2`).
- **Country + region are read SERVER-side** (`CF-IPCountry`, `CF-IPRegionCode`, ISO 3166-2), never sent from the client. Time, version (blob3) and platform (blob4) MAY be combined and filtered (Destin, 2026-09-13); country and region are NEVER cross-tabulated or filtered by anything (fingerprint risk at low cell counts) — `/countries` and `/regions` ignore every filter param.
...
- **CF Analytics Engine SQL is a narrow subset, NOT full ClickHouse** — `count(DISTINCT)` only, quoted `INTERVAL '30' DAY`, no JOIN/UNION/CTE; FROM-subqueries work; `toStartOfWeek` returns SUNDAYS despite its docs. 422 gotchas: `docs/worker-backend.md`.
