...
## Binary viewers & security — guard: `read-binary-access.test.ts`, `ipc-channels.test.ts`
- **`artifacts:read-binary` is GUARDED on desktop** (`read-binary-access.ts`, reachable over remote WS): reads restricted to project roots + tracked externals, well-known secret dirs refused even inside roots, 50MB gate. Don't bypass. Binary viewers load bytes via this IPC (base64) — NEVER `fetch('file://…')`.
...
- **Android artifact list/project stubs return `{ok:false,error:'not-implemented-on-mobile'}` — that's the contract.** `project:*` IPC is desktop-only (context read/write allow-listed to discovered set). Parity pinned by `ipc-channels.test.ts`.
