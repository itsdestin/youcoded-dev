...
**Invariant:** never leave a React tree mounted when a test ends; `tests/setup-dom.ts` does
it for every jsdom file. Don't disable it.
**Why:** React work queued when jsdom is torn down throws `window is not defined`, reported
as an *Unhandled Error*: the run fails while every test shows passed, so the red names
nothing.
...
**Why:** under load the work hasn't started, the code takes the *other* branch and fails as
though the feature were broken. ~100 remain; convert any you touch.
**Guard:** none — candidate.
...
**Why:** a timeout cuts a run mid-flight, so it *looks* like a logic bug — and vitest's 5s
default is a unit-test budget for files that import 4,000-line modules.
**Guard:** none — candidate.
...
(env override, e.g. `YOUCODED_EVAL_RUNS_DIR`) — never snapshot-and-restore a real directory
— and removes that root with a retrying remove (`maxRetries`), not `rmSync`.
**Why:** a fire-and-forget write landing mid-removal throws `ENOTEMPTY` into a test that
already passed · `docs/testing-under-load.md`.
...
`global-setup.ts` creates, wipes and — by RETURNING a teardown — removes it. **No
import-time filesystem side effect** (`knip` imports it).
...
asserted an error string the tool never emits; one asserted totals, so deleting a code path
left every assertion true. Each certified its own fixtures.
**Guard:** source-scanning tests have one — `tests/helpers/guard-scope.ts` (~20 use it):
`readStripped()` drops comments, so an assertion cannot match the WHY note quoting the old
code, and `assertPatternMatches(pattern, knownPositive, label)` fails a pattern matching
nothing. Anchor to the ONE thing you mean: bare `toContain('disabled={hostOnly}')` passed
with that prop deleted — two other controls carry it.

## Before calling a failure "flake"
Run it in isolation (passes → load-sensitive) **and** in a pristine `origin/master`
worktree (still fails → pre-existing); one idle run proves little. A deterministic failure everywhere is
usually machine state. **A red CI leg you have not read is not noise.**
