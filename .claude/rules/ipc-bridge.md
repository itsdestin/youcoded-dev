...
- **Transcript parser:** canonical `desktop/src/main/transcript-watcher.ts`, Kotlin mirror `parser/TranscriptWatcher.kt`. The old two-implementation `TranscriptSource` design and its fixtures are gone; reintroducing one means landing parity fixtures too.

## Settings → Development — guard: `ipc-channels.test.ts`
- **`ipc-channels.test.ts` holds the authoritative `dev:*` list — SEVEN types today, not six** (`dev:diagnostics` joined `log-tail`, `summarize-issue`, `submit-issue`, `install-workspace`, `install-progress`, `open-session-in`), in parity across `preload.ts`, `remote-shim.ts`, `ipc-handlers.ts` and `SessionService.kt`. The guard reads only three of the four surfaces (`ipc-handlers.ts` uses `IPC.DEV_*` constants); `remote-server.ts` bridges none.
