...
- **Voice typing adds three; the list stays closed.** `voice.sendAudio` — the phone's recogniser owns the microphone. `voice.micAccess` — the Activity's launcher owns that question. The `voice` namespace itself, absent from `remote-shim.ts` outside `android-local` — a browser grants no microphone without an encrypted connection (deck Q-7). Guards: `ipc-channels.test.ts`, `remote-shim-voice-gate.test.ts`.
