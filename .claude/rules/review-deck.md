...
approval and sign-off is a step on it.
...
Other, with a note, is on every step. Several designs of one thing are ONE Choice step, never
a yes/no each. A step whose picture exists in one theme carries its own `themes`.

## A question deck is pages he scrolls
**Invariant:** questions share one page until a marker (`{"id", "page", "intro"}`) starts the
next; add one only where the thinking shifts. **Why:** "my mindset should stay in the same
...
by field name. **Why:** structure in the tool, not a session's memory — 28 of 28
options drifted in two days. **Guard:** `test_words.py`.

## Look at it before he does
**Invariant:** `preview <spec>`, then READ `preview/contact.png`, before `serve`. **Why:** four
decks went out in a day carrying a visible defect. **Guard:** none — candidate (`test_cli.py`
covers the command, not the habit).

## A change under `deck/` is reviewed on a deck
**Invariant:** an edit to `page.css`, `page.js`, `page.html.tmpl` or `deck/fixture/` ships only
after `selfie` has shown him before and after it. **Why:** the deck is a surface, reviewed like
every other one. **Guard:** none — candidate.

## The deck opens on the theme his app is on
**Invariant:** never hand-order `themes` — `build`/`serve` read the app's; `--theme` overrides,
`"theme": "fixed"` pins. **Why:** every deck opened on Midnight while his app was on Golden
Sunbreak. **Guard:** `test_spec.py`.
