...
---
# Lists, timelines, hidden tabs — draw only what can be seen
...
- **`React.memo` cannot stop a component that reads a context itself.** A hidden-but-mounted tab is `hidden` prop AND memo AND stable props AND no context read of its own — the parent passes the one value down (FilesTab / `ArtifactContext`). Accepted limit: `MarketplaceCard` reads `useMarketplace()`, so an install redraws the visible cards. Guard: ast-grep `filestab-mounted-with-hidden-prop`, `filestab-memoized`, `filestab-no-artifact-context`; `project-view-files-tab-stays-mounted.test.tsx`.
