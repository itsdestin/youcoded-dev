...
- **The hire subject IS the grant width; the CATALOG stamps `grantScope`** (D1/D2): built-in `${charter}:${workDir}`; `user` folders `${charter}:file:${id}@${fp}` (every project); `project` folder `${charter}:${workDir}:file:${id}@${fp}`; `fp` = sha256 of the file, so an edit re-asks. Auto-edit appends `{Task, '*:file:*', ask}` after its broad allow; a `task_id` resume is gated by the ledger's spawn-time `definitionFingerprint`; `createTaskTool` memoises `roster.resolve`. Guards: `task-tool`, `permission-engine`, `native-session-host`, `describe-rule` tests.
