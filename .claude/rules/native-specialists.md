...
- **A file-defined hire's subject carries `:file:<id>` and its card offers no Always-allow** (built-ins keep the old subject); the card shows one provenance line and looks up a definition per cwd, refetching once on a miss.
