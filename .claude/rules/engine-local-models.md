...
- **The memory pool is the ENGINE's own first GPU device** (`poolFromDevices`, off the marker), so an integrated GPU scores against its real pool; `gpu-detector`'s VRAM probe is only the FALLBACK, biased to null (→ RAM-only). **A unified pool (Apple Silicon; an AMD APU = carve-out + GTT, told apart by kfd) is flagged `sharedMemory` and never added to free RAM** — guard: `fit-estimator.test.ts` / `gpu-detector.test.ts`. Working memory comes from the GGUF header, never a flat allowance.
