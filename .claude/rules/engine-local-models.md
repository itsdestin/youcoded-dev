...
verify:
  - path: youcoded/desktop/src/main/models/fit-estimator.ts
    contains: "Math.min[(]poolFree, input.availableBytes[)]"
  - test: youcoded/desktop/tests/quant-parser.test.ts
  - test: youcoded/desktop/tests/gpu-detector.test.ts
  - test: youcoded/desktop/tests/fit-estimator.test.ts
...
A supervised, SHA-256-verified `llama-server` in router mode, plus the model manager. **READ `youcoded/docs/engine-dependencies.md` first — every fact below is verified there against b10665. On a bump re-run the NINE bump-gate probes `test-engine/README.md` names; a new one MUST pass `--models-dir`.**

## Engine (Plan B, `src/main/engine/`)
- **`--models-dir <cacheDir>` discovers GGUFs — NOT `LLAMA_CACHE`**. Without it `GET /models` is empty and every completion 400s. Router id = filename minus `.gguf` (== `cache-scan.ts`) — **except a vision model, named by its FOLDER**.
- **`models.ini` (`--models-preset`) is the most dangerous file the app writes** — one bad key or unparsable line is FATAL at startup, killing EVERY local model at the next spawn with nothing on screen. Write it ONLY through `model-presets.ts`; the supervisor retries without the bad section, then without the file.
...
- **The memory pool is the ENGINE's own first GPU device** (`poolFromDevices`, off the marker), so an integrated GPU scores against its real pool; `gpu-detector`'s VRAM probe is only the FALLBACK, biased to null (→ RAM-only); its `shared` flag (Apple Silicon, Linux AMD APU) still applies: fit uses the SMALLER of pool and free RAM, capped at physical RAM. Working memory comes from the GGUF header, never a flat allowance.
- **The quant parser keeps `mmproj` + any separator-bounded `mtp` token off the quant LIST but RETURNS the projector**, so every quant carries it.
- **Faster-engine opt-in:** CUDA on Windows x64 / NVIDIA (plus a `cudart-` archive); ROCm on Windows x64 and Linux x64 / AMD, with the chip's gfx target checked against THAT ROW's list on LINUX ONLY. **`rocm-prereqs.ts` does not gate the offer** — it picks the BUTTON: "Set up" when AMD's software is missing, "Switch" when it is there.
