...
3. `python3 scripts/ui-review/review-cards.py serve <spec>` **in the background** (it builds
   ...
   needed a `measure` line before the Before run). It never opens a browser. **The address to give Destin is the `[deck] http://127.0.0.1:<port>/<out>.html` line `serve` prints** (also in `<spec>.serve.json` next to the spec) — quote it whole, in chat, as the last line of your turn; the bare port now redirects to the deck, but never guess a URL you have not read. Destin answers
