...
**A dependency master added after your worktree was made is simply absent from its hardlinked `node_modules`** (found 2026-09-11: merging master brought an `import 'dompurify'` that `tsc` could not resolve, and no checkout on disk had it installed). `npm install` would write `node_modules/.package-lock.json` in place through the shared inode (`docs/PITFALLS.md` → Worktrees), so install it into fresh inodes instead: `npm pack <pkg>@<version in package-lock.json>` in a scratch folder, `mkdir node_modules/<pkg>`, then `tar -xzf <file>.tgz -C node_modules/<pkg> --strip-components=1`; repeat for any optional types it names, and check `stat -c %h` on a copied file reads 1.
