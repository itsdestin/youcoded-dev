...
#   cd mockups && python3 -m http.server 8901   # view at localhost:8901/compare.html
set -e
cd "$(dirname "$0")"
...
cp ../compare.html .
# round-1 mockups are static history (not generated by build.py) but keep tabs A/B/C alive
cp ../mockup-a-editorial.html ../mockup-b-theater.html ../mockup-c-bento.html .
echo "ready — now: python3 ../build.py && python3 -m http.server 8901"
