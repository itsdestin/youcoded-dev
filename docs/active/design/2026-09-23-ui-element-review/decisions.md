...
| Menu rows | **Plain rows** in menus (right-click and similar): no box, no line; hover highlights. | Long lists (Settings) and switchers re-asked in lists-2: the first lists deck showed "Today" identical to one option unlabelled, which confused R-1/R-3/R-4. | `ui-element-review-lists#R-2` |
