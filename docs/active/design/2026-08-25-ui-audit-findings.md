...
| # | Proposal | Touches | Risk / what users will notice |
|---|---|---|---|
| P-6 | **[measured]** Settings is unreachable from the welcome screen without expanding the form. Welcome screen: *New Session* uses the real primary treatment from P-12; *Resume Session* stays secondary; add a Settings gear to this screen's corner (today Settings is unreachable here without expanding the form). | Welcome | A gear appears on the welcome screen. **P-12 rejected → P-6 is now only the Settings gear; the primary treatment stays as is.**  **Decided 2026-08-27 (`phase-d-brief.html`): YES, revised** — not a lone gear: the welcome screen gets the same bare frame the Terminal view has (Settings gear, Projects folder, minimize/maximize/close) — no session switcher, no status-bar chips, no chat bar.  **Review 2026-08-27 (`phase-d-review.html`): YES as built** (BareHeaderBar).  **SHIPPED 2026-08-27 — youcoded #339.** |
| P-19 | **[measured]** chip names truncate after ~4 characters; text below the floor; broken thumbnails. Composer attachment chips: icon · name (≥ 12 characters before truncating) · always-visible ✕, `md` radius, `text-2xs`; broken thumbnails fall back to the file-type icon. | composer | Attachments become readable.  **Decided 2026-08-27: OTHER — mock-ups first.** Several chip designs, one per file kind, before anything is built: name as a small strip at the bottom of the card; a real preview for most kinds where cheap (at least image / markdown / text); a plan for every other kind.  **Mock-ups 2026-08-27 (`phase-d-mockups.html`): PICK C** (128×96 card, preview on top, 12 px name strip, ✕ always visible) — plus: "we should make all tiles (here and in project view/elsewhere where they show up) render with proper markdown styling instead of being able to see the ## and whatever else." Text previews render as formatted markdown, not raw source; same rule wherever a tile previews a file.  **BUILT 2026-08-27** (`phase-d-review3.html`): `AttachmentChip` (128×96, preview / name strip / ✕ always on), rendered-markdown previews via `MarkdownContent preview` (no buttons/links inside a tile — a `<button>`-in-`<button>` React error on every Projects screen was caught by the sweep and fixed), new `fs:read-head` bridge on all five surfaces (600 B default, 4 KB hard cap). Other tiles: ArtifactThumbnail now renders markdown; raw one-liners in ContextTab/ConversationsTab/ConversationPreview and the Marketplace `longDescription` listed for follow-up.  **Review 3 (2026-08-27): YES as built.**  **SHIPPED 2026-08-27 — youcoded #339.** |
| P-14 | **[measured]** the find bar covers the first message. Find bar gets its own `panel` surface anchored top-right *above* the transcript instead of inside the user bubble. | find | The bar stops covering the first message.  **Decided 2026-08-27: YES** — the find bar gets its own row above the messages (browser-style); the chat shifts down while it is open.  **Review 2026-08-27: OTHER** — "i still want it to be its own element/pill with border, i just don't want it to overlap my message" → the in-flow row is now a transparent lane that reserves the height, with the same bordered pill at its right edge (round 2 deck).  **Review 2 (2026-08-27 21:37): YES** — lane + bordered pill.  **SHIPPED 2026-08-27 — youcoded #339.** |
| P-9 | **[mixed]** measured: category pills are the smallest text in the app; judgment: the rest. Skills drawer: category pills use the shared filter-pill shape/size; the two icon buttons leave the search field and sit beside it as `size="icon"` buttons with names; the *Add Skills* card becomes an `EmptyState` centred in the row when there are no skills, and a normal last card when there are. | Skills drawer | Search field looks like every other one.  **Decided 2026-08-27: pills YES (14 px, Marketplace shape, Favorites-only in the row); icon buttons NO — they stay inside the search field but must show their name/action on hover; Add Skills YES** (last card of the same grid; standard empty message when nothing is installed).  **Review 2026-08-27: pills YES, Add Skills YES, empty state YES as built** (shared `FilterChip`; Favorites-only lights solid accent).  **SHIPPED 2026-08-27 — youcoded #339.** |
| P-20 | **[measured]** the terminal grid stops at 2/3 of the pane; unreadable over wallpapers. Terminal: the terminal grid fills the pane width; the terminal surface takes the ≥ 0.9 `panel` opacity guarantee from P-16 under wallpaper packs. | Terminal | No more empty right third; readable over wallpapers.  **Decided 2026-08-27: OTHER on both.** Width: may be a capture-environment artefact (the terminal was also too short in that shot; Destin has not seen either in his app) — reproduce in a dev instance before building anything. Wallpaper backing: he wants to see a few renderings (opacity variants) before committing.  **Mock-ups 2026-08-27: backing OTHER** — "kinda confused about the difference between scrim and stronger panel" — explained in chat (scrim = the terminal itself drawn less see-through; panel = a coloured sheet behind it) → **PICK: panel, at 80% (not 90)** — "fine with 3 at 8-% instead of 90".  **P-20.2 BUILT 2026-08-27**: `computeTerminalSurface` — wallpaper/gradient themes get `--terminal-backing: var(--panel)` and opacity floored at 0.8; flat themes byte-identical. **P-20.1 REPRODUCED in a dev instance 2026-08-27** (`scratch/ui-phase-d-electron/…/e2-terminal-view.png`, Ivory Schematic, 1440×900): Claude Code's text and input line stop at ~950 px — the PTY is not told the real column count (hypothesis: the initial fit is skipped while the grid is 0×0 behind the chat view, and the later resize never reaches the PTY). Filed as a ROADMAP bug; not fixed in Phase D.  **Review 3 (2026-08-27): P-20.2 YES as built.**  **SHIPPED 2026-08-27 — youcoded #339.** |
...
| # | Proposal | Touches | Risk / what users will notice |
|---|---|---|---|
| P-4 | **[mixed]** measured: renders the word `undefined`, no scrollbar; judgment: button vocabulary. Model Providers: one button vocabulary per card (secondary chips for *Add key/Test*, `danger-outline` *Remove*), *Add provider* becomes a secondary button (not full-width outline), never render `undefined` (fall back to "—"), dialog scroll shows a scrollbar. | Model Providers | None visible beyond consistency. **DECLINED 2026-08-28 — both halves.** The `undefined` no longer renders: the local-models list was rewritten by the download-resume work (#341), verified in a fresh capture before the brief was written, so that half had nothing left to fix. The scroll cue was measured — 588 px of dialog holding ~900 px of content, bottom edge dimming text by about a quarter, no bar — and shown as its own step; Destin: **no** (a bar would land in every scrolling panel in the app for one dialog's complaint). The button-vocabulary half was shown as judgment against a neutral baseline (4 shapes in one dialog, `Add provider` 386 px against 60–80 px): **no**. |
| P-7 | **[measured]** rows wrap in a dialog too narrow for its own content. Keyboard Shortcuts → `panel` (420px) size; rows grid-aligned (label column + key column); dialog scrolls with a visible scrollbar. | one dialog | Wider dialog.  **Worse than filed, found 2026-08-28:** the dialog was `prompt` + `scrollBody={false}`, and `prompt` caps at 476 px while thirteen rows need ~550 px — with no scroll region anywhere, the last THREE shortcuts (`/`, `Escape`, `Arrow Left/Right`) were unreachable at every window size, not merely clipped. **YES 2026-08-28, SHIPPED (youcoded #347):** shared header + scrolling body at `panel`, labels and keys in two grid columns, one label trimmed by a word to hold one line. 13/13 rows readable, was 10; rows a uniform 29 px, were 33 and 45. Guard: `scripts/ast-grep/rules/shortcuts-dialog-keeps-scroll-body.yml` (retired `menu-row-reachability.test.ts`, Plan B 2026-09-16). |
| P-8 | **[measured]** rows wrap to two lines. All-sessions popover width becomes `min(28rem, 88vw)` and rows truncate to one line with a title tooltip; scroll cue via bottom fade. | one popover | Rows stop wrapping.  **YES 2026-08-28** with a redesign of the row, settled over three rounds. R1 (as filed): name one truncated line with the full name on hover, project moved to a second line under it, list cap 336 → 432 px so 8.5 rows fit where 6.5 did. R2 (Destin's sketch): name and project share the left edge; the bare dot becomes a **named status pill** at the right of the name (`STATUS_LABEL`, new in `StatusDot.tsx`); a folder mark before the project; tag marks under the pill. Amber has no name in his four, so it was put as an A/B — **pick A, five words** ("Needs a Look" keeps its own meaning; folding it into red's "Needs Input" would have two colours saying one thing). R3: **tags as named chips, not dots** — "tags should not be dots, but full chips with spelled names" — capped at three plus a `+N`, note page-mark after. Tag data is one `session:get-meta` per open row (the live list carries none; a bulk read would need a new channel on five surfaces). **SHIPPED (youcoded #347)**; the Shift switcher is the same menu, so §6b #22 is fixed with it. |
...
| E | P-4, P-7, P-8 | contained dialogs/popovers | main | **SHIPPED 2026-08-28 (youcoded #347)** — P-7 (three unreachable shortcuts + wrapping), P-8 (row rebuilt: one-line name, project + folder mark, named status pill, tag chips); P-4 declined in full (its `undefined` was already gone; a visible scroll bar would touch every panel). Decks: `phase-e-brief`, `phase-e-review`, `phase-e-review2`, `phase-e-review3` |
