...
| `tests/chatgpt-oauth.test.ts` | chatgpt-types.ts has no toLocaleString; StatusBar imports formatTime12 | M, C | split (keep OAuth/limit behaviour) | eslint `no-restricted-properties` + tsc — corrected in u1 (2026-09-16): lint is oxlint now (`.oxlintrc.json`, same rule); the import half became ast-grep `limit-sentence-uses-shared-time-format` + `status-bar-uses-shared-format-time12`, because tsc accepts a local re-implementation with the import deleted (the old `not.toMatch(/function formatTime12/)`). REVIEW FIX (u1 round 1, item 1, 2026-09-16): the oxlint ban only sees a direct `.toLocaleTimeString()` property access, missing an indirect string key, a `Reflect.get` lookup, a bare-call import and an aliased import specifier's original name — added ast-grep `chatgpt-types-no-locale-formatter` as the backstop | 1/29 (29 sites, not 28; the one case is half behaviour — its source half is removed, the case stays) |
| `tests/close-prompt-meta-unreadable.test.tsx` | Renders prompt; two slice get-meta handlers for `unreadable` | M, D | split (render cases stay) | `get-meta-marks-failed-read-unreadable` (+ `-remote` twin: the two hosts are different shapes, one per file — u1, 2026-09-16). REVIEW FIX (u1 round 1, item 2, 2026-09-16): the no-store branch only accepted a bare `kind: object` return and false-fired on a legitimate `return { …, unreadable: … } as Meta` / `satisfies Meta` — added an `as_expression`/`satisfies_expression` branch | 2/5 |
...
| `tests/dev-load-recovery.test.tsx` | main.ts uses shared MOUNT_PROBE_JS rather than an inline copy | M, C | split (keep the two jsdom probe cases) | tsc (import presence) — corrected in u1 (2026-09-16): ast-grep `main-uses-shared-mount-probe`, because tsc accepts both evasions the case forbade (a same-name local `const MOUNT_PROBE_JS` with the import deleted — typecheck passed on exactly that inversion — and an inline-string call with the import left unused). REVIEW FIX (u1 round 1, item 3, 2026-09-16): the required-call pattern was plain-dot only and false-fired on `win.webContents?.executeJavaScript(MOUNT_PROBE_JS)` (an optional-chain receiver) — added a matching `?.` pattern branch | 1/3 |
| `tests/dialog-shell.test.tsx` | Renders Dialog; also scans renderer for hand-rolled OverlayPanel | M (A) | split (keep render+width cases) | `no-hand-rolled-dialog-shell` + `no-hand-rolled-dialog-shell-exemption-still-applies` (review of u2, 2026-09-16: both rules named) | 3/12 |
| `tests/explainer-shell.test.tsx` | Renders explainer; two scan hosts for chrome props/onBack | M, A | split (render cases stay) | `settings-explainer-no-chrome-props` + `explainer-hosts-pass-onback-showinfo` + `explainer-hosts-pass-onback-showinfo-lifted` (review of u2, 2026-09-16: all three rules named) | 2/5 |
...
| `tests/filter-menu-chip.test.tsx` | Renders chips; three grep ResumeBrowser for shared primitives | M, A | split (render cases stay) | `resume-browser-uses-shared-filter-chips` (+ `resume-browser-no-local-filter-chip-parts` for the absence halves, u2 2026-09-16) | 3/9 |
| `tests/folders-service.test.ts` | Real service behaviour, plus two transports-call-the-service greps | M (C) | split (keep service cases) | `no-folders-json-outside-service` + `folders-service-called-by-both-transports` (u3: the presence half needed its own rule) | 2/7 |
| `tests/primitive-adoption.test.ts` | ui/ primitives adopted; no hand-rolled toast or black/white wash | A, G | split (keep CSS coupling) — u3 correction: the two adoption cases ("every ui/ primitive is used outside components/ui/", "the intentionally-unadopted list does not rot") are kept too: a run-time export list checked against every other file is cross-file, which no rule expresses | `no-hand-rolled-toast` (+ `-ts`), `toast-auto-dismiss-owned-by-primitive` (App.tsx timer half), `no-literal-black-white-wash` (+ `-ts`) | 2/5 converted; 3/5 kept |
...
| `tests/project-watcher.test.ts` | Watcher behaviour; one pins SKIP_DIRS set parity | M, E | split (parity case stays — u3: kept as a readSource text read with a WHY; set equality across two files, one unexported, is not a rule) | — | 1/14 |
...
| `tests/resume-browser-organize.test.tsx` | Card behaviour; one derives classNames from SessionDrawer source | M, G | split (interaction cases stay) — review-of-u6 correction: trim, not delete — the case's source-free half (R5-1: both rename cues visible at rest) stays as a render case; only the SessionDrawer scraping goes | — | 1/10 |
...
| `tests/runtime-default.test.tsx` | One file owns the storage key; forms init from defaultRuntime() | M, D | split (keep the four defaultRuntime unit cases) | `runtime-default-key-single-owner` (+ `-tsx`, `-present`); CORRECTION (u7): case (f) — the forms' seed and post-create reset — needs its own rule, `new-session-forms-use-default-runtime` | 2/6 |
...
| `tests/shell-session.test.ts` | Shell spawn behaviour; four grep main files for call shapes | M, B | split (spawn/validator cases stay) | `run-in-terminal-chunked-write` (u8: + `run-in-terminal-entry-points-validate`(`-remote`), `pty-worker-passthrough-single-write`, `remote-session-create-refuses-shell-provider`) | 4/27 |
| `tests/shell-session-renderer.test.ts` | App.tsx/HeaderBar contain exact shell-session gating expressions | M, B, D | split (keep label/chip/pty-gate/slash-routing cases) | `shell-session-view-forced` (u8: + `shell-session-view-toggle-hidden`, `shell-session-no-claude-ui`, `shell-session-permission-cycle-guarded`, `runtime-union-has-no-shell`, `session-strip-runtime-never-shell`) | 13/22 |
...
| `tests/status-strip-authority.test.tsx` | Renders StatusStrip; scans centred status lines, hardcoded error fallbacks | M (A) | split (keep 4 render cases) | `no-hardcoded-error-fallback` | 5/9 — corrected (u9, 2026-09-16): the 5 are two per-file-count scans, two danger-zone reads and a scope-size check. Rules `no-hardcoded-error-fallback`, `no-centred-status-paragraph`, `danger-zone-has-danger-callout`, `danger-zone-no-fixed-status-red`; the scope check deleted; the two count scans KEPT as text (trimmed to their exempt files only — an exact per-file count is not rule-expressible) |
| `tests/statusline-rate-limits.test.ts` | Shipped hook scripts never name credentials/bearer; Android copy identical | M, E | split (keep parity + cache-contract behaviour) | — | 4/14 — u9 (2026-09-16): the 4 are Part 1's cases; 3 deleted per `—` (usage-fetch.js absence, credentials/Keychain/Bearer names, USAGE_FETCH/toolkit_root), the parity case kept as a readSource text read with a WHY. NOTE: the deleted three were the only automated guard on the OAuth-token ban (hook-scripts/ and app assets are outside check.sh's scan dirs) |
| `tests/tag-list-host.test.ts` | Service behaviour plus two handler-body greps for empty-list fallback | M (D) | split (keep 3 behaviour cases) | `tags-list-no-empty-fallback` (+ `-remote` twin: the two hosts are different shapes, one per file — u9, 2026-09-16) | 2/5 |
