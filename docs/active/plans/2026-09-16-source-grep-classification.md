...
| `tests/accepted-history-privacy.test.ts` | Ciphertext sentinel absent from every reader; one file names private dir | M, D | split (keep the driven-session sentinel sweep) | `private-continuation-dir-single-owner` | 1/1 |
| `tests/animation-frame-budget.test.ts` | CSS/inline animations carry steps(), no unbudgeted infinite | A, G | CORRECTION (t6a, 2026-09-16): only the file's FIRST describe block, "perpetual animations are frame-budgeted" (15 cases), matches this row's description. Its second describe block, "motion vocabulary" (30 cases), is an unrelated SessionStrip drag/hover/label/easing-token source-text pin set — it is the guard `.claude/rules/session-strip-motion.md` names by file path, so it stays in this file untouched. Applied disposition, first block only: split (13 CSS/per-file cases → new tests/animation-css-budget.test.ts via readSource, 2 generic TSX sweep cases → ast-grep) | `no-unstepped-infinite-animation` | 15/45 (of the first block's 15, all 15 read source; 2 convert to the rule, 13 move to the new file; the other 30 cases are the unrelated kept block, untouched) |
| `tests/arcade-authority.test.ts` | Game files avoid attention APIs, palette classes, shared state | M, D, A | CORRECTION (t6a, 2026-09-16): of the 13 source-reading cases, only the row's named rule case (the 5 loop-expanded "assistant-finishing" cases, 1 `it(` site) converts; 2 Class-H non-vacuity self-tests ("the palette pattern actually matches a known positive", "the game-word list actually matches a known positive") are deleted, nothing left to guard; the remaining 10 (state.play ownership, shared-state vocabulary, challenge wiring, ChatView key yield, game key-claim, palette ban, main-process vocabulary ban, arcade-handler no-ranking, resize-effect ordering ×2) are each narrow, low-reuse, single-or-two-file text pins — kept via readStripped, unchanged, rather than writing 6+ more bespoke one-off ast-grep rules for one row of eight assigned | `arcade-no-forbidden-attention-apis` (+ `-ts` twin) | 3/17 converted-or-deleted (1 rule site + 2 deletes); 14/17 kept (4 registry-shape + 10 text-read) |
| `tests/attention-banner.test.tsx` | Renders banner states; one case greps ChatView provider prop | M | split (render cases stay) | `chatview-passes-provider-to-attentionbanner` | 1/14 |
...
| `tests/buddy-caption-channel.test.ts` | Window-move and setTitle calls occur only inside place() | M, D | split (keep the driven window-manager tests) | `buddy-window-move-only-in-place` | 3/22 |
| `tests/buddy-consent-gate.test.ts` | Gate logic unit tests; six grep main.ts BUDDY_SHOW wiring | M | split (gate/cache cases stay) | `buddy-show-consults-refusal-gate` | 6/20 |
| `tests/callout-authority.test.tsx` | Renders Callout; three count hand-rolled tinted blocks | M, A | split (render cases stay) | `no-hand-rolled-callout-tint` | 3/7 |
| `tests/chatgpt-oauth.test.ts` | chatgpt-types.ts has no toLocaleString; StatusBar imports formatTime12 | M, C | split (keep OAuth/limit behaviour) | eslint `no-restricted-properties` + tsc | 1/28 |
| `tests/close-prompt-meta-unreadable.test.tsx` | Renders prompt; two slice get-meta handlers for `unreadable` | M, D | split (render cases stay) | `get-meta-marks-failed-read-unreadable` | 2/5 |
...
| `tests/dev-load-recovery.test.tsx` | main.ts uses shared MOUNT_PROBE_JS rather than an inline copy | M, C | split (keep the two jsdom probe cases) | tsc (import presence) | 1/3 |
...
| `tests/filter-chip.test.tsx` | MarketplaceFilterBar/CommandDrawer carry no local chip recipe | M, A | split (keep the three render cases) | `no-local-filter-chip-recipe` | 2/6 |
