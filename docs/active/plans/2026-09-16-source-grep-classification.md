...
| `tests/primitive-adoption.test.ts` | ui/ primitives adopted; no hand-rolled toast or black/white wash | A, G | split (keep CSS coupling) — u3 correction: the two adoption cases ("every ui/ primitive is used outside components/ui/", "the intentionally-unadopted list does not rot") are kept too: a run-time export list checked against every other file is cross-file, which no rule expresses | `no-hand-rolled-toast` (+ `-ts`), `toast-auto-dismiss-owned-by-primitive` (App.tsx timer half), `no-literal-black-white-wash` (+ `-ts`) | 5/5 |
