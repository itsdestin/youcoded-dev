...
> **MOOT (verified 2026-07-16):** the legacy sync-service pull/push demolition (youcoded `b5d7c95b` and follow-ups) already removed this block — `grep youcoded-core desktop/src/main/sync-service.ts` returns zero matches. Nothing to do; skip this task.
...
> **Already clean (verified 2026-07-16):** neither `desktop/src/shared/bundled-plugins.ts` nor `app/.../skills/BundledPlugins.kt` contains `youcoded-core`. Steps 1–3 reduce to a verification grep; Step 4 (PITFALLS example check) may still apply.
...
### Task 18b: Android + renderer/shared dead-branch sweep (added by 2026-07-16 audit)

**Context:** The original Phase 2 tasks only covered desktop main-process files. A full grep on 2026-07-16 found the same dead branches mirrored on Android, plus renderer/shared surface the plan never enumerated. Same gate as the rest of Phase 2.

**Android mirrors (each mirrors a desktop task):**
- [ ] `app/src/main/kotlin/com/youcoded/app/skills/PluginInstaller.kt:63,68` — drop the `|| sourceMarketplace == "youcoded-core"` disjuncts (mirror of Task 12)
- [ ] `app/src/main/kotlin/com/youcoded/app/skills/SkillScanner.kt:63,69` — drop the `startsWith("youcoded") → "youcoded-core"` source branch (mirror of Task 13); update `app/src/test/kotlin/.../SkillScannerTest.kt` fixtures (`youcoded-core at top level produces bare skill ids` test models the defunct clone layout)
- [ ] `app/src/main/kotlin/com/youcoded/app/skills/MarketplaceFileReader.kt:14,87,120` — layered core/life/productivity layout comments + depth tuning (mirror of Task 15)
- [ ] `app/src/main/kotlin/com/youcoded/app/skills/LocalSkillProvider.kt:499,504` — rename `youcoded-core-output-styles.json` → `youcoded-output-styles.json` with the SAME one-shot migration as desktop's Task 16 (must land in the same release as Task 16 or the two platforms read different config files)
- [ ] `app/src/main/kotlin/com/youcoded/app/skills/SkillConfigStore.kt:92,101` — `youcoded-core-$layer` legacy-layer handling is dead (the 3-layer clone no longer exists on disk post-cleanup)

**Renderer/shared surface (skill source union):**
- [ ] `desktop/src/shared/types.ts:232` — narrow `source: 'youcoded-core' | 'self' | 'plugin' | 'marketplace'` (pairs with Task 13's skill-scanner narrowing)
- [ ] `desktop/src/renderer/App.tsx:1515` — assigns `source: 'youcoded-core'`
- [ ] `desktop/src/renderer/components/SkillCard.tsx:35,68-73` — youcoded-core badge style + label branch
- [ ] `desktop/src/renderer/components/CommandDrawer.tsx:148` — comment reference
- [ ] Note: `app/src/main/assets/web/App.js` is a build artifact — regenerated by `scripts/build-web-ui.sh`, not hand-edited

**Deliberate keepers (do NOT remove in Phase 2 — they ARE the migration code):**
- `legacy-cleanup.ts` / `legacy-cleanup.test.ts`, `Bootstrap.kt` cleanup + stale-entry prune, `install-hooks.js:229` + `Bootstrap.kt:1135` statusline legacy-path detection. These get their own removal decision several releases after N+1.
- `symlink-cleanup.test.ts` fixtures seed `youcoded-core` paths as *legacy inputs being cleaned* — still-valid test data, judge case-by-case.

---

### Task 18c: Decide the theme `source: 'youcoded-core'` vocabulary (added by 2026-07-16 audit)

**Context:** Themes use `source: 'youcoded-core'` as a REGISTRY DATA VALUE meaning "official/built-in theme" — not a code branch: `desktop/src/renderer/themes/theme-types.ts:146,151`, `desktop/src/shared/theme-marketplace-types.ts:29-30,70`, `desktop/src/renderer/state/theme-context.tsx:20-23`, `desktop/src/renderer/themes/theme-asset-resolver.ts:22,25`, plus `wecoded-themes` registry entries and the sync.js never-overwrite rule (`docs/registries.md`). Renaming touches registry data compatibility with themes already published/cached.

- [ ] **Destin decides:** rename the value (e.g. `'official'`, with a read-compat shim for cached/published registry data) OR accept it as permanent vocabulary residue.
- [ ] If residue: amend Task 19 Step 4's "minimal or zero matches" expectation to exclude the theme-source vocabulary files.
...
> **RE-GATED (2026-07-16 audit): defer Tasks 24–27 until Phase 3.** The original gate assumed youcoded-core would be effectively frozen after Phase 1. It isn't: until Release N ships, existing installs (≤ v1.2.4) still run hooks from the clone, and mirror fixes still land in the repo (e.g. write-guard exit-code fix, youcoded-core PR #119, 2026-07-15). So sessions still need the checkout (`setup.sh` clone stays), the still-accurate `.claude/rules/youcoded-toolkit.md` + `docs/toolkit-structure.md` stub stay (both are already deprecation-aware; the "defunct 3-layer" doc this task assumed was slimmed to a stub earlier), and the `build-and-release.md` toolkit flow stays (it still describes how mirror fixes reach users). What WAS safe ran on 2026-07-16: CLAUDE.md's false bundled-plugin claims were fixed on sight (see execution log). Run Tasks 24–27 as the last step of Phase 3, after the archive.
...
- **2026-07-16 — full-plan audit + safe-work pass.** Findings applied to the plan: Task 14 is MOOT (sync demolition already removed the branch), Task 18 is already clean on both platforms, and two tasks were ADDED — Task 18b (Android mirrors + renderer/shared `source` union the original Phase 2 never enumerated) and Task 18c (theme `source: 'youcoded-core'` registry-vocabulary decision, blocks Task 19's zero-grep gate). Phase 4 re-gated to Phase 3 (youcoded-core is still the live hook channel for existing installs — see the RE-GATED note). Work shipped: **youcoded PR #147** (merged) — `theme-preview-sync.test.ts` had been reading theme-preview.css from the youcoded-core checkout, but theme-builder moved to `wecoded-marketplace/wecoded-themes-plugin/` long ago, so all 34 drift-guard assertions were SILENTLY SKIPPING via `describe.skipIf`; repointed at the live copy, added a location canary so a future move fails loudly, and updated one stale assertion for the chrome-glass refactor (35/35 green). Workspace CLAUDE.md's false claims (youcoded-core "auto-installed on launch" via bundled-plugins.ts — it was removed there in Phase 1 and the app now deletes the clone) fixed on sight, and the both-copies hook-fix invariant documented there. Write-guard copies verified: app bundles are byte-identical and functionally in sync with youcoded-core HEAD (one comment-line diff), pinned by `write-guard-contract.test.ts`.
