...
supersedes:
  ...
  - 23 ROADMAP entries (full original texts: git history of ROADMAP.md before the 2026-07-22 consolidation commits; index in §10)
design-refs:
...
6. **Multimodal round-trip — attach, Read, display** (absorbs three entries: multimodal input 2026-05-19, Read-tool filetypes 2026-07-22, markdown-image display idea 2026-07-19). One story, three legs:
   ...
   b. **Read tool.** Correction to the folded entry (verified on master 2026-07-22): Read is NOT silent-mojibake — it has a NUL-byte binary sniff with a loud refusal (`tools/read.ts:48` "Cannot read X: it is a binary file") and a 50 MB cap with an honest hint. The real gaps: the schema says just "Read a file from the filesystem" (no text-only statement, so models try images and eat a refusal), and refusal isn't *parity* — CC's Read returns images/PDFs. Ship: honest schema wording now; image support for vision-capable bindings (image parts in the tool result; non-vision bindings keep the refusal); decide whether harness tools get `artifacts.readBinary`-grade access, respecting the `read-binary-access.ts` sensitive-path denylist.
   c. **Display:** a model showing the user an image currently works ONLY by accident in packaged builds — react-markdown's default URL transform passes `/tmp/...` through, and the `file://` origin resolves it; dev 404s the same markdown, and it bypasses the app's one deliberate image path (`ImageView.tsx:23`/`ArtifactThumbnail.tsx:112-121` read bytes via `readBinary` into same-origin `blob:` URLs precisely to avoid `file://`). Make it real: an `img` override resolving local absolute paths through `readBinary`→`blob:` (fixes dev/prod parity, one image mechanism), `max-width:100%; height:auto`, and a deliberate `urlTransform` decision (any renderer-readable path renders today — fine for trusted model output, a live consideration for pasted/untrusted transcripts). Render image Read results in ToolCards via the same path. Then document the capability in the native system prompt ("you can show the user an image by …"). The renderer half is provider-agnostic (CC replies hit the same markdown path) — it lives here because the native prompt is what advertises it.
