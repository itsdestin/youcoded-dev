...
- **Prerequisite:** Plan A merged in both repos (`git -C <worktree> log --oneline origin/master | grep -c 'retire ci-red-vs-master'` prints `1`). Master is protected: a red Linux check blocks your PR, so read failures, never re-run and hope.
