...
- Footage is filmed at **1920×1200** (device scale 1 — the spec's `scale: 2` is dropped: the window is downscaled to fit the 1080p frame, so a 4 % push-in never exceeds 1:1) and the phone at **390×844** with `platform=android`.
...
- `scripts/promo/footage/`, `scripts/promo/out/`, `scripts/promo/public/` (generated) are gitignored; `scripts/promo/node_modules` is covered by the root ignore.
...
| `scripts/ui-review/record.mjs` (exists) | Gains the `Space` key and the `autoplay` action. |
| `scripts/ui-review/autoplay.mjs` | Pure loop: poll a JS predicate, press a key when true. |
| `scripts/ui-review/autoplay.test.mjs` | `node --test` for the loop. |
| `scripts/ui-review/scenes/promo-*.json` (9 files) | One scene per storyboard beat. |
| `scripts/promo/film.sh` | Boots the workbench for a worktree, records every promo scene into `scripts/promo/footage/`, writes the footage-review page. |
| `scripts/promo/package.json`, `remotion.config.ts`, `tsconfig.json` | The Remotion project. |
| `scripts/promo/src/index.ts`, `Root.tsx`, `Promo.tsx` | Entry, composition registration, the timeline. |
...
| `scripts/promo/src/beats/Beat1.tsx … Beat8.tsx` | One component per storyboard beat. |
...
- [ ] **Step 6: Listen check for Destin**

Run: `cd scripts/promo/music && python3 song.py promo /tmp/claude-1000/-home-destin-youcoded-dev/921a8e54-65a6-411f-afeb-1bda78496140/scratchpad/music/promo.wav && ffmpeg -y -loglevel error -i .../promo.wav -codec:a libmp3lame -q:a 2 .../promo.mp3`
Then play it: `mpv --no-video --really-quiet .../promo.mp3`. Tell Destin the section times (from the grid) so he can say which part, if any, needs changing. Do not proceed to Task 7 until he has heard it.
...
### Task 2: The recorder learns `Space` and `autoplay`

**Files:**
- Create: `scripts/ui-review/autoplay.mjs`
- Create: `scripts/ui-review/autoplay.test.mjs`
- Modify: `scripts/ui-review/record.mjs` — the `KEYS` table (line ~135) and the action dispatch loop (line ~159-181)
- Modify: `scripts/ui-review/README.md` — the actions table under "Recording a loop"

**Interfaces:**
- Produces: scene action `{"autoplay": {"ms": 9000, "every": 25, "when": "<js expression → boolean>", "key": "Space", "minGap": 120}}` — for `ms` milliseconds, evaluate `when` every `every` ms and press `key` when it returns true, never more often than `minGap` ms. `{"key": "Space"}` now works.
- `autoplay.mjs` exports `runAutoplay({ evaluate, press, sleep, now, ms, every, when, minGap })` → resolves `{ presses: number, polls: number }`.

- [ ] **Step 1: Write the failing test**

`scripts/ui-review/autoplay.test.mjs`:
```js
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { runAutoplay } from './autoplay.mjs';
...
test('never presses when the predicate stays false', async () => {
  ...
  const r = await runAutoplay({ ...c, ms: 500, every: 50, minGap: 0,
...
test('a throwing predicate counts as false and does not abort the loop', async () => {
  ...
  await runAutoplay({ ...c, ms: 200, every: 50, minGap: 0,
...
Run: `node --test scripts/ui-review/autoplay.test.mjs`
Expected: FAIL — `Cannot find module './autoplay.mjs'`.

- [ ] **Step 3: Write `autoplay.mjs`**

```js
// Autoplay for record.mjs: poll a JS predicate inside the page and press a key
...
export async function runAutoplay({ evaluate, press, sleep, now, ms, every = 25, when, key = 'Space', minGap = 120 }) {
  const end = now() + ms;
...
```
In the action loop, add a branch before `else if (a.eval)`:
```js
  else if (a.autoplay) {
    const r = await runAutoplay({
      evaluate, press: (k) => key(k), sleep, now: () => Date.now(),
      ms: a.autoplay.ms, every: a.autoplay.every, when: a.autoplay.when, key: a.autoplay.key, minGap: a.autoplay.minGap,
    });
    console.error(`autoplay: ${r.presses} presses over ${r.polls} polls`);
  }
```
Add one row to the README's actions table:
```
| `autoplay` (`ms`, `when`, `key`, `every`, `minGap`) | poll `when` (a JS expression evaluated in the page) every `every` ms for `ms` ms and press `key` when it is true — the recorder "plays" a game by reading the DOM |
```

- [ ] **Step 6: Verify nothing else broke**

Run: `node --check scripts/ui-review/record.mjs && node --test scripts/ui-review/autoplay.test.mjs && node scripts/check-doc-commands.mjs --list | head -3`
Expected: no syntax error, 3 passing, the doc-command lister runs.

- [ ] **Step 7: Commit**

```bash
git add scripts/ui-review/autoplay.mjs scripts/ui-review/autoplay.test.mjs scripts/ui-review/record.mjs scripts/ui-review/README.md
git commit -m "feat(ui-review): record.mjs learns Space and an autoplay action that plays by reading the DOM"
```
...
- Modify: `.gitignore` (append `scripts/promo/footage/`, `scripts/promo/out/`, `scripts/promo/public/`)

**Interfaces:**
- Produces: `scripts/promo/footage/<scene>.webm` + `.webp` for every scene, and `docs/active/prototypes/promo-2026-09/footage-review.md` linking the posters.
...
{ "base": "http://127.0.0.1:5473/?mode=workbench&child=1&scenario=site&latency=150&seed=none&reply=briefing&title=new+session",
  "theme": "midnight", "width": 1920, "height": 1200, "boot": 3000,
  "actions": [
    {"hold": 900},
    {"clickText": "Briefing", "tag": "button", "settle": 500},
...
{ "base": "http://127.0.0.1:5473/?mode=workbench&child=1&scenario=site&latency=150&seed=none&reply=sheet&title=Q3+sales",
  "theme": "midnight", "width": 1920, "height": 1200, "boot": 3000,
  "storage": { "youcoded-drawer-width": 900 },
  "actions": [
    {"hold": 600},
    {"eval": "dispatchEvent(new CustomEvent('buddy:attach-file',{detail:{filePath:'/home/you/Documents/Q3-sales.xlsx'}}))", "settle": 900},
...
{ "base": "http://127.0.0.1:5473/?mode=workbench&child=1&scenario=default&latency=150",
  ...
  "actions": [
    ...
    {"drag": "[data-session-idx='4']", "to": "[data-session-idx='1']", "ms": 1100, "settle": 400},
    {"hold": 1600}
  ] }
```
`promo-remote.json`:
...
{ "base": "http://127.0.0.1:5473/?mode=workbench&child=1&scenario=site&latency=150&remote=setup",
  ...
  "actions": [
    ...
    {"click": "[title='Remote Access']", "settle": 1200},
    {"waitForText": "Set Up Remote Access", "tag": "button", "timeout": 8000},
    {"clickText": "Set Up Remote Access", "tag": "button", "settle": 1200},
    {"hold": 2600}
  ] }
```
...
{ "base": "http://127.0.0.1:5473/?mode=workbench&child=1&scenario=site&latency=150&reply=week-sync&autoplay=2500&platform=android",
  "theme": "midnight", "width": 390, "height": 844, "boot": 3000,
  "actions": [
    {"hold": 2600},
    {"waitForText": "synced to your other devices", "tag": "p", "settle": 800},
...
{ "base": "http://127.0.0.1:5473/?mode=workbench&child=1&scenario=site&latency=150&reply=week&lease=held%3APixel%209",
  ...
  "actions": [
    ...
    {"waitForText": "take over here", "timeout": 8000, "settle": 1500},
    {"clickText": "Take over", "tag": "button", "settle": 1200},
    {"hold": 2600}
  ] }
```
`promo-theme.json`:
...
{ "base": "http://127.0.0.1:5473/?mode=workbench&child=1&scenario=site&latency=150&reply=theme-builder&seed=none&title=new+theme",
  ...
  "actions": [
    ...
    {"eval": "window.__workbenchAppearanceSync({theme:'golden-sunbreak'})", "settle": 200},
    {"hold": 3800}
  ] }
```
...
OUT="$HERE/footage"; mkdir -p "$OUT"
REVIEW="$WS/docs/active/prototypes/promo-2026-09"; mkdir -p "$REVIEW/footage"

STARTED=0
trap '[ "$STARTED" = 1 ] && pkill -f "[v]ite --port $WB_PORT" || true' EXIT
if ! curl -sf "http://127.0.0.1:$WB_PORT/" >/dev/null; then
  STARTED=1
  (cd "$TDIR" && nohup bash "$WS/scripts/run-workbench.sh" "$TDIR" >"$OUT/workbench.log" 2>&1 &)
...
{
  echo "# Promo footage — review"; echo; echo "Filmed $(date -I) from \`$TDIR\`. One poster (the last frame) per scene; the clips are in \`scripts/promo/footage/\`."; echo
...
For `promo-flappy`, the `autoplay:` stderr line reports presses. Watch the WebM (`mpv scripts/promo/footage/promo-flappy.webm`) — the bird must clear at least four pipes. If it dies early, tune the `when` expression's `0.02` altitude margin (raise to flap earlier) and the `minGap` (lower for quicker recovery); re-film only that scene: `bash scripts/promo/film.sh promo-fakes promo-flappy`.

- [ ] **Step 4: Hand the posters to Destin**

Tell him the page path `/home/destin/youcoded-dev/docs/active/prototypes/promo-2026-09/footage-review.md` and the clips folder. Ask him to flag any scene that looks wrong. Re-film flagged scenes before Task 8.

- [ ] **Step 5: Commit** (scenes, script, gitignore, the review page and its posters — the WebMs stay out)

```bash
git add scripts/ui-review/scenes/promo-*.json scripts/promo/film.sh .gitignore docs/active/prototypes/promo-2026-09/
...
**Interfaces:**
- Consumes: `scripts/promo/public/promo.wav`, `promo.grid.json`, `sfx-*.wav` (copied from the music render), `scripts/promo/public/footage/*.webm` (symlink or copy of `footage/`), `scripts/promo/src/rig.ts` (copied from the app).
- Produces: composition id `Promo`, 1920×1080, 30 fps, `durationInFrames = barFrame(34)` (2075).
...
{ "compilerOptions": { "target": "ES2022", "module": "ESNext", "moduleResolution": "Bundler", "jsx": "react-jsx",
    "strict": true, "skipLibCheck": true, "resolveJsonModule": true, "noEmit": true }, "include": ["src"] }
EOF
npm install
mkdir -p public/footage src/beats
cp ../../youcoded/desktop/src/renderer/components/mascot/default-buddy-rig.ts src/rig.ts
cp ../../youcoded/desktop/src/renderer/themes/community/golden-sunbreak/assets/mascot-welcome.svg src/golden-welcome.svg.txt
cp ../../youcoded/desktop/src/renderer/themes/community/golden-sunbreak/assets/mascot-shocked.svg src/golden-shocked.svg.txt
```
Check `src/rig.ts` has no imports (it exports one string constant); if it imports anything, inline what it needs. The two `.svg.txt` files are read with `?raw`-free imports below by inlining: open each and paste its contents into `src/golden.ts` as `export const GOLDEN_WELCOME = \`…\`; export const GOLDEN_SHOCKED = \`…\`;`.

Then stage the inputs:
```bash
cd scripts/promo/music && python3 song.py promo ../public/promo.wav && cd .. && ln -sfn ../footage public/footage
```
...
export const beatFrame = (b: number, beat: number) => Math.round((b * BAR_S + beat * BEAT_S) * FPS);
export const TOTAL_FRAMES = barFrame(TOTAL_BARS);
/** Beat boundaries from the spec's storyboard, in bars. */
export const BEATS = { b1: [0, 2], b2: [2, 6], b3: [6, 10], b4: [10, 14], b5: [14, 16], b6: [16, 23], b7: [23, 29], b8: [29, 34] } as const;
export const barsOf = (k: keyof typeof BEATS) => BEATS[k][1] - BEATS[k][0];
export const framesOf = (k: keyof typeof BEATS) => barFrame(BEATS[k][1]) - barFrame(BEATS[k][0]);
```

- [ ] **Step 3: Visual primitives**

`src/Backdrop.tsx` — a slow gradient in the theme's colours:
```tsx
import { AbsoluteFill, useCurrentFrame, interpolate } from 'remotion';
export const THEMES = {
  midnight: { canvas: '#0D1117', panel: '#161B22', accent: '#B1BAC4', glow: '#1f2a3a' },
  golden:   { canvas: '#08080e', panel: '#140e1a', accent: '#ffc030', glow: '#3a2410' },
};
export const Backdrop: React.FC<{ theme: keyof typeof THEMES }> = ({ theme }) => {
  const f = useCurrentFrame(); const t = THEMES[theme];
...
```
`src/Window.tsx` — the laptop-window chrome the footage sits in (a shadowed rounded panel; the app draws its own title bar, so this is only a shadow + edge):
```tsx
import React from 'react';
export const Window: React.FC<{ scale?: number; y?: number; children: React.ReactNode }> = ({ scale = 0.86, y = 0, children }) => (
  <div style={{ position: 'absolute', left: '50%', top: '50%', width: 1920, height: 1200,
    transform: `translate(-50%, -50%) translateY(${y}px) scale(${scale})`, transformOrigin: 'center',
    borderRadius: 18, overflow: 'hidden', boxShadow: '0 40px 120px rgba(0,0,0,.55), 0 0 0 1px rgba(255,255,255,.06)' }}>
    {children}
  </div>
);
```
...
```tsx
import React from 'react';
export const Phone: React.FC<{ x: number; y: number; scale?: number; children: React.ReactNode }> = ({ x, y, scale = 0.9, children }) => (
  <div style={{ position: 'absolute', left: x, top: y, width: 390, height: 844, transform: `scale(${scale})`, transformOrigin: 'top left',
    borderRadius: 44, overflow: 'hidden', border: '10px solid #0b0b0f', boxShadow: '0 30px 80px rgba(0,0,0,.6)' }}>
    {children}
  </div>
);
```
`src/Caption.tsx` — the one line, springing up from below on its cue frame:
```tsx
import { useCurrentFrame, useVideoConfig, spring, interpolate } from 'remotion';
import { loadFont } from '@remotion/google-fonts/Inter';
const { fontFamily } = loadFont();
export const Caption: React.FC<{ text: string; at: number; big?: boolean }> = ({ text, at, big }) => {
  const f = useCurrentFrame(); const { fps } = useVideoConfig();
  const s = spring({ frame: f - at, fps, config: { damping: 14, stiffness: 120 } });
  if (f < at) return null;
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 72, textAlign: 'center', fontFamily,
      fontSize: big ? 84 : 52, fontWeight: 700, letterSpacing: '-0.02em', color: '#fff',
      textShadow: '0 4px 24px rgba(0,0,0,.6)', opacity: s, transform: `translateY(${interpolate(s, [0, 1], [40, 0])}px)` }}>
      {text}
    </div>
  );
};
```
`src/Footage.tsx` — a clip inside the window, trimmed in frames, with an optional push-in:
```tsx
import { OffthreadVideo, staticFile, useCurrentFrame, interpolate } from 'remotion';
import { Window } from './Window';
export const Footage: React.FC<{ file: string; from?: number; pushIn?: number; y?: number }> = ({ file, from = 0, pushIn = 0, y = 0 }) => {
  const f = useCurrentFrame();
  const scale = 0.86 * (1 + interpolate(f, [0, 240], [0, pushIn], { extrapolateRight: 'clamp' }));
  return (
    <Window scale={scale} y={y}>
      <OffthreadVideo src={staticFile(`footage/${file}.webm`)} trimBefore={from} muted style={{ width: 1920, height: 1200 }} />
    </Window>
  );
};
```

- [ ] **Step 4: `src/poses.ts` and `src/Mascot.tsx`** — the host

`src/poses.ts` (copied values from the app's `mascot-poses.ts`; pivots from the rig's `data-pivot`s):
```ts
...
import { GOLDEN_WELCOME, GOLDEN_SHOCKED } from './golden';
import { PIVOT, POSES, type Pose } from './poses';

/** A cue: from `at`, the mascot is at (x, y) in the given pose. Positions and limb
 *  angles spring from the previous cue with the app's own constants (k=170, d=16). */
export type Cue = { at: number; x: number; y: number; pose: Pose; size?: number; costume?: 'default' | 'golden' };

const SPRING = { stiffness: 170, damping: 16, mass: 1 };

export const Mascot: React.FC<{ cues: Cue[]; accent: string; onAccent: string }> = ({ cues, accent, onAccent }) => {
  const f = useCurrentFrame(); const { fps } = useVideoConfig();
  const i = cues.findLastIndex((c) => c.at <= f);
  if (i < 0) return null;
  const cur = cues[i], prev = cues[i - 1] ?? cur;
  const s = spring({ frame: f - cur.at, fps, config: SPRING });
  const x = interpolate(s, [0, 1], [prev.x, cur.x]), y = interpolate(s, [0, 1], [prev.y, cur.y]);
  const size = cur.size ?? 160;
  const p = POSES[cur.pose], pp = POSES[prev.pose];
  const armL = interpolate(s, [0, 1], [pp.arms[0], p.arms[0]]);
  // The wave: the raised arm oscillates ±18° at ~3 Hz, like the rig's waveSway.
  const wave = p.wave ? Math.sin((f - cur.at) / fps * 2 * Math.PI * 3) * 18 : 0;
  const armR = interpolate(s, [0, 1], [pp.arms[1], p.arms[1]]) + wave;
  // Idle breathing: a 2 % bob at 0.4 Hz, always on.
  const bob = Math.sin(f / fps * 2 * Math.PI * 0.4) * size * 0.02;

  if (cur.costume === 'golden') {
    const art = cur.pose === 'shocked' ? GOLDEN_SHOCKED : GOLDEN_WELCOME;
    return <div style={{ position: 'absolute', left: x, top: y + bob, width: size, height: size, color: accent }}
      dangerouslySetInnerHTML={{ __html: art }} />;
  }
  const css = `
    #rig-root { transform-box: view-box; }
    #rig-arm-left  { transform-box: view-box; transform-origin: ${PIVOT['rig-arm-left']};  transform: translate(0,0.5px) rotate(${armL.toFixed(2)}deg); }
    #rig-arm-right { transform-box: view-box; transform-origin: ${PIVOT['rig-arm-right']}; transform: translate(0,0.5px) rotate(${armR.toFixed(2)}deg); }
    [id^="rig-face-"] { display: none; }
    #rig-face-${p.face} { display: inline; }
  `;
  return (
    <div style={{ position: 'absolute', left: x, top: y + bob, width: size, height: size,
      ['--rig-accent' as string]: accent, ['--rig-on-accent' as string]: onAccent }}>
      <style>{css}</style>
...
```
`findLastIndex` needs `lib: ["ES2023", "DOM"]` in `tsconfig.json`; add it.

- [ ] **Step 5: The beats**

Each beat is `AbsoluteFill` + `Footage` + `Caption` + (optional) mascot cues exported for the timeline. `src/beats/index.ts` exports the eight components and their cue lists; frames inside a beat are **relative to the beat's first frame** (TransitionSeries resets `useCurrentFrame` per sequence). Illustrative full code for beats 1, 4, 6, 7; beats 2, 3, 5, 8 follow the same shape with the spec's caption and file:

`src/beats/Beat1.tsx` — cold open: window rises on the riser, wordmark, mascot peeks then hops onto the title bar.
```tsx
import { AbsoluteFill, useCurrentFrame, useVideoConfig, spring, interpolate, Audio, staticFile } from 'remotion';
import { Backdrop } from '../Backdrop';
import { Footage } from '../Footage';
import { Caption } from '../Caption';
import { Mascot, type Cue } from '../Mascot';
import { barFrame } from '../grid';
export const CUES1: Cue[] = [
  { at: 0, x: 880, y: 1100, pose: 'peek' },            // below the frame
  { at: 10, x: 880, y: 960, pose: 'peek' },             // peeks up
  { at: 34, x: 880, y: 960, pose: 'curious' },          // looks around
  { at: barFrame(1), x: 640, y: 150, pose: 'idle' },    // hops onto the title bar as the window rises
];
export const Beat1: React.FC = () => {
  const f = useCurrentFrame(); const { fps } = useVideoConfig();
  const rise = spring({ frame: f - barFrame(1), fps, config: { damping: 18, stiffness: 90 } });
  return (
    <AbsoluteFill>
      <Backdrop theme="midnight" />
      <div style={{ opacity: rise }}><Footage file="promo-idle-midnight" y={interpolate(rise, [0, 1], [700, 40])} /></div>
      <Caption text="YouCoded" at={4} big />
      <Caption text="Useful. Fun. Yours." at={barFrame(1) + 10} />
      <Sequence from={10} durationInFrames={12}><Audio src={staticFile('sfx-pop.wav')} volume={0.6} /></Sequence>
      <Sequence from={barFrame(1) + 12} durationInFrames={12}><Audio src={staticFile('sfx-pop.wav')} volume={0.6} /></Sequence>
      <Mascot cues={CUES1} accent="#B1BAC4" onAccent="#0D1117" />
    </AbsoluteFill>
  );
};
```
(`Sequence` is imported from `remotion` alongside the others; a pop plays when the mascot first peeks and when it lands on the title bar.)

`src/beats/Beat4.tsx` — Flappy: the host is hidden (the bird is the mascot).
```tsx
import { AbsoluteFill } from 'remotion';
import { Backdrop } from '../Backdrop';
import { Footage } from '../Footage';
import { Caption } from '../Caption';
// Trim: the recording spends ~4 s typing before the game opens; start on the Games click.
export const Beat4: React.FC = () => (
  <AbsoluteFill>
    <Backdrop theme="midnight" />
    <Footage file="promo-flappy" from={110} pushIn={0.04} />
    <Caption text="Play while it works." at={12} />
  </AbsoluteFill>
);
```
`src/beats/Beat6.tsx` — three clips in one beat (bars 16–23 = 7 bars): remote (bars 16–18), phone slides in (18–20), takeover (20–23).
```tsx
import { AbsoluteFill, Sequence, useCurrentFrame, useVideoConfig, spring, interpolate, OffthreadVideo, staticFile } from 'remotion';
import { Backdrop } from '../Backdrop';
import { Footage } from '../Footage';
import { Caption } from '../Caption';
...
import { Mascot, type Cue } from '../Mascot';
import { barFrame } from '../grid';
const T1 = barFrame(2), T2 = barFrame(4), T3 = barFrame(7);
export const CUES6: Cue[] = [
  { at: 0, x: 640, y: 150, pose: 'idle' },
  { at: T1 + 6, x: 1500, y: 120, pose: 'curious', size: 120 },   // hops onto the phone
  { at: T2 + 6, x: 640, y: 150, pose: 'idle' },                  // and back
];
export const Beat6: React.FC = () => {
  const f = useCurrentFrame(); const { fps } = useVideoConfig();
  const slide = spring({ frame: f - T1, fps, config: { damping: 16, stiffness: 110 } });
  return (
    <AbsoluteFill>
      <Backdrop theme="midnight" />
      <Sequence from={0} durationInFrames={T2}><Footage file="promo-remote" from={40} /></Sequence>
      <Sequence from={T2} durationInFrames={T3 - T2}><Footage file="promo-takeover" from={60} /></Sequence>
      <Sequence from={T1} durationInFrames={T2 - T1 + 20}>
        <Phone x={interpolate(slide, [0, 1], [1980, 1420])} y={90}>
          <OffthreadVideo src={staticFile('footage/promo-phone.webm')} muted style={{ width: 390, height: 844 }} />
        </Phone>
      </Sequence>
      <Caption text="Start on your laptop. Finish on your phone." at={T1 + 8} />
      <Mascot cues={CUES6} accent="#B1BAC4" onAccent="#0D1117" />
    </AbsoluteFill>
  );
};
```
`src/beats/Beat7.tsx` — the theme drop: the recording already contains the switch; find the frame where the theme flips (the first frame whose window colour is gold — measure it once with `ffmpeg -i promo-theme.webm -vf "select='gt(scene,0.3)',showinfo" -f null -` and read the `pts_time`) and trim so the flip lands on **frame 0 of the beat** (= bar 23's downbeat). The mascot changes costume on the same frame with a shocked → welcome cue, and the chime plays.
```tsx
...
import { Caption } from '../Caption';
import { Mascot, type Cue } from '../Mascot';
export const FLIP_FRAME_IN_RECORDING = 0; // ← set from the showinfo measurement (frames at 24 fps × 30/24)
export const CUES7: Cue[] = [
  { at: 0, x: 640, y: 150, pose: 'shocked', costume: 'golden' },
  { at: 18, x: 640, y: 150, pose: 'welcome', costume: 'golden' },
];
export const Beat7: React.FC = () => (
  <AbsoluteFill>
    <Backdrop theme="golden" />
    <Footage file="promo-theme" from={FLIP_FRAME_IN_RECORDING} pushIn={0.03} />
    <Audio src={staticFile('sfx-chime.wav')} volume={0.7} />
    <Caption text="Describe a look. It's yours." at={14} />
    <Mascot cues={CUES7} accent="#ffc030" onAccent="#000" />
  </AbsoluteFill>
);
```
The pre-flip half of the theme story (typing the request, the tool cards) belongs to the **end of beat 6's slot**? No — the spec keeps beat 7 at bars 23–28 starting on the flip. So the typing plays in the last two bars of the build: in `Promo.tsx`, beat 6 is bars 16–21 and a short `Beat7a` (bars 21–23, footage `promo-theme` from frame 0, no caption) precedes `Beat7`. Add `Beat7a.tsx` accordingly (same shape as Beat4 without a caption) and give beat 6 `T3 = barFrame(5)`.

`src/beats/Beat2.tsx` — the quick chip. The recording spends ~0.9 s idle before the chip click; trim so the click lands near the downbeat.
```tsx
import { AbsoluteFill } from 'remotion';
import { Backdrop } from '../Backdrop';
import { Footage } from '../Footage';
import { Caption } from '../Caption';
import { Mascot, type Cue } from '../Mascot';
export const CUES2: Cue[] = [{ at: 0, x: 640, y: 150, pose: 'idle' }];
export const Beat2: React.FC = () => (
  <AbsoluteFill>
    <Backdrop theme="midnight" />
    <Footage file="promo-quick-chip" from={18} pushIn={0.03} />
    <Caption text="Start with one click." at={10} />
    <Mascot cues={CUES2} accent="#B1BAC4" onAccent="#0D1117" />
  </AbsoluteFill>
);
```
`src/beats/Beat3.tsx` — the spreadsheet, two trims of one recording: the request and the tool cards, then (on bar 3 of the beat) the re-opened, sorted sheet. `AFTER_FRAME` is the first frame after the second `Q3-sales.xlsx` click; measure it once with `ffmpeg -i footage/promo-sheet.webm -vf "select='gt(scene,0.25)',showinfo" -f null -` (the last scene change) and convert `pts_time` to frames at 30 fps.
```tsx
...
import { Mascot, type Cue } from '../Mascot';
import { barFrame } from '../grid';
export const AFTER_FRAME = 0; // ← from the showinfo measurement
export const CUES3: Cue[] = [
  { at: 0, x: 640, y: 150, pose: 'idle' },
  { at: 30, x: 1180, y: 150, pose: 'curious' },      // leans toward the panel as it opens
];
export const Beat3: React.FC = () => (
  <AbsoluteFill>
    <Backdrop theme="midnight" />
    <Sequence from={0} durationInFrames={barFrame(3)}><Footage file="promo-sheet" from={20} /></Sequence>
    <Sequence from={barFrame(3)}><Footage file="promo-sheet" from={AFTER_FRAME} pushIn={0.04} /></Sequence>
    <Caption text="Your files, right beside the chat." at={12} />
    <Mascot cues={CUES3} accent="#B1BAC4" onAccent="#0D1117" />
  </AbsoluteFill>
);
```
`src/beats/Beat5.tsx` — the drag. The break in the music is quiet; no push-in, so the motion reads.
```tsx
import { AbsoluteFill } from 'remotion';
import { Backdrop } from '../Backdrop';
import { Footage } from '../Footage';
import { Caption } from '../Caption';
import { Mascot, type Cue } from '../Mascot';
export const CUES5: Cue[] = [
  { at: 0, x: 640, y: 150, pose: 'idle' },
  { at: 24, x: 640, y: 150, pose: 'curious' },      // head turns as the pill goes by
];
export const Beat5: React.FC = () => (
  <AbsoluteFill>
    <Backdrop theme="midnight" />
    <Footage file="promo-strip" from={24} />
    <Caption text="Drag your conversations into order." at={8} />
    <Mascot cues={CUES5} accent="#B1BAC4" onAccent="#0D1117" />
  </AbsoluteFill>
);
```
`src/beats/Beat7a.tsx` — the theme request being typed (bars 21–23, under the build). No caption; the caption belongs to the drop.
```tsx
import { AbsoluteFill } from 'remotion';
import { Backdrop } from '../Backdrop';
import { Footage } from '../Footage';
import { Mascot, type Cue } from '../Mascot';
export const CUES7A: Cue[] = [{ at: 0, x: 640, y: 150, pose: 'idle' }];
export const Beat7a: React.FC = () => (
  <AbsoluteFill>
    <Backdrop theme="midnight" />
    <Footage file="promo-theme" from={12} />
    <Mascot cues={CUES7A} accent="#B1BAC4" onAccent="#0D1117" />
  </AbsoluteFill>
);
```
`src/beats/Beat8.tsx` — the close. Two caption lines; the mascot waves, then hops out of the top of the frame on bar 3 of the beat.
```tsx
...
import { Mascot, type Cue } from '../Mascot';
import { barFrame } from '../grid';
export const CUES8: Cue[] = [
  { at: 0, x: 640, y: 150, pose: 'welcome', costume: 'golden' },
  { at: barFrame(3), x: 640, y: -320, pose: 'welcome', costume: 'golden' },
];
export const Beat8: React.FC = () => (
  <AbsoluteFill>
    <Backdrop theme="golden" />
    <Footage file="promo-idle-golden" from={10} y={40} />
    <Caption text="Free. Open source. Windows · Mac · Linux · Android." at={6} />
    <Sequence from={barFrame(1)}>
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 28, textAlign: 'center', fontFamily: 'Inter', fontSize: 30, color: '#ffc030', letterSpacing: '0.02em' }}>github.com/itsdestin/youcoded</div>
    </Sequence>
    <Sequence from={barFrame(3)} durationInFrames={12}><Audio src={staticFile('sfx-pop.wav')} volume={0.5} /></Sequence>
    <Mascot cues={CUES8} accent="#ffc030" onAccent="#000" />
  </AbsoluteFill>
);
```
`src/beats/index.ts`:
```ts
export { Beat1 } from './Beat1'; export { Beat2 } from './Beat2'; export { Beat3 } from './Beat3'; export { Beat4 } from './Beat4';
export { Beat5 } from './Beat5'; export { Beat6 } from './Beat6'; export { Beat7a } from './Beat7a'; export { Beat7 } from './Beat7'; export { Beat8 } from './Beat8';
```
Beat 6's third clip (the takeover) starts at `T3 = barFrame(5)` is wrong now that the beat spans bars 16–21: use `T1 = barFrame(2)` (phone slides in), `T2 = barFrame(3)` (cut to the takeover recording), and the beat's own length `barFrame(5)` as the end — update the constants in `Beat6.tsx` to `const T1 = barFrame(2), T2 = barFrame(3), T3 = barFrame(5);`.

- [ ] **Step 6: `src/Promo.tsx`, `Root.tsx`, `index.ts`**
...
import { barFrame } from './grid';
import { Beat1, Beat2, Beat3, Beat4, Beat5, Beat6, Beat7a, Beat7, Beat8 } from './beats';

const CUT = 6; // frames — a 200 ms slide; the spec caps transitions at 250 ms
const seq = (bars: [number, number], node: React.ReactNode) => (
  <TransitionSeries.Sequence durationInFrames={barFrame(bars[1]) - barFrame(bars[0]) + CUT}>{node}</TransitionSeries.Sequence>
);
const cut = (dir: 'from-right' | 'from-bottom' = 'from-right') => (
  <TransitionSeries.Transition timing={linearTiming({ durationInFrames: CUT })} presentation={slide({ direction: dir })} />
);
export const Promo: React.FC = () => (
  <AbsoluteFill style={{ background: '#0D1117' }}>
    <Audio src={staticFile('promo.wav')} />
    {/* a whoosh on every cut: one per transition, placed on its start frame */}
    {[2, 6, 10, 14, 16, 21, 23, 29].map((b) => (
      <Sequence key={b} from={barFrame(b) - 2} durationInFrames={10}><Audio src={staticFile('sfx-whoosh.wav')} volume={0.35} /></Sequence>
    ))}
    <TransitionSeries>
      {seq([0, 2], <Beat1 />)}{cut()}
      {seq([2, 6], <Beat2 />)}{cut()}
      {seq([6, 10], <Beat3 />)}{cut()}
      {seq([10, 14], <Beat4 />)}{cut('from-bottom')}
      {seq([14, 16], <Beat5 />)}{cut()}
      {seq([16, 21], <Beat6 />)}{cut()}
      {seq([21, 23], <Beat7a />)}
      <TransitionSeries.Transition timing={linearTiming({ durationInFrames: 2 })} presentation={fade()} />
      {seq([23, 29], <Beat7 />)}{cut()}
      {seq([29, 34], <Beat8 />)}
    </TransitionSeries>
  </AbsoluteFill>
...
```
Note the `+ CUT` on every sequence: a transition overlaps its neighbours by its length, so each sequence is padded by one cut to keep every beat's first frame on its downbeat. The last sequence is padded too; the composition's `durationInFrames` is `TOTAL_FRAMES` and the tail is clipped.
...
- [ ] **Step 7: Typecheck and draft render**

Run: `cd scripts/promo && npm run typecheck && npm run render:draft`
Expected: `out/draft.mp4` (960×540) in a few minutes. Watch it (`mpv out/draft.mp4`) and check: every cut lands on a downbeat (the kick), the flip in beat 7 is on bar 23's first frame, no caption overlaps the app's composer, the mascot never covers a tool card that is being read.

- [ ] **Step 8: Hand the draft to Destin**

Give him `/home/destin/youcoded-dev/scripts/promo/out/draft.mp4`. Collect notes as a list; apply; re-render the draft. Repeat until he says it's right.

- [ ] **Step 9: Commit** (sources only)

```bash
git add scripts/promo/package.json scripts/promo/package-lock.json scripts/promo/remotion.config.ts scripts/promo/tsconfig.json scripts/promo/src
git commit -m "feat(promo): the Remotion timeline — eight beats on the beat grid, the mascot host, captions, sound"
```
...
- [ ] `docs/roadmap/dev-workspace.md` under `## rigs`: one line — "promo video is re-renderable: `bash scripts/promo/film.sh <worktree> && bash scripts/promo/render.sh`; re-film after any UI change the video shows".
