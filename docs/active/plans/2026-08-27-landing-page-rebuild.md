...
1. `One app, any AI` · `Pick the AI. Keep the conversation.` · `Claude with your plan. Hundreds of models through OpenRouter. Or a model that runs on your own computer — free, offline, private. Switch mid-conversation; nothing is lost.` · `row1-any-ai`
2. `It actually does things` · `Give it a folder. It asks before it touches anything.` · `It reads your files, writes new ones, runs the steps, and searches the web. Every action that changes something shows up as a question first — and every "always allow" you grant is listed in one screen where you can take it back.` · `row2-does-things`
3. `Your projects, in one place` · `Files, chats, and instructions — together.` · `Open spreadsheets, PDFs, documents, and images without leaving the app. Edit a file next to the conversation about it. Search everything you've ever said in a project.` · `row3-projects`
4. `Stay organized` · `Tags, notes, and one-tap shortcuts.` · `Tag and annotate conversations, filter by tag, pin the ones that matter. Quick chips run the things you do every day in one tap. A project's instructions are a tab you can read and edit, not a hidden file.` · `row4-organized`
5. `Follow you everywhere` · `Start on your laptop. Finish on your phone.` · `Windows, macOS, Linux, Android, and any browser. Your conversations and files stay in sync through your own private GitHub, so what you started here is waiting there.` · `row5-follow` (+ phone, see Step 4)
...
8. `For builders` · `Claude Code inside, your own agent beside it.` · `Run Claude Code as a first-class session next to the app's own agent. Review, stage, and commit changes without leaving the window. Connect tools over MCP. Download and run local models with a GPU-fit check.` · `row8-builders`
