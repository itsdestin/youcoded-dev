...
Lesson for cluster work: `git rm` the old files per target, right before that target's commit — four of the 2026-09-18 cluster agents staged every deletion up front and their first commit swept all of them in (caught before push each time). Lesson for the remaining merges: `tsconfig.tests.json` conflicts are deletions on both sides — keep an entry only if BOTH sides kept it, comparing without the trailing comma (the last list entry has none, and a naive line compare dropped a still-excluded file once).
