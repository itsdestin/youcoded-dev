...
| `tests/app-quit-routes.test.ts` | 2 | 0 (file deleted) | `registers teardown on all three quit routes`; `routes every quit through one idempotent teardown` | ast-grep rule `main-registers-all-quit-routes` |
...
| `tests/appearance-buddy-broadcast.test.ts` | 1 | 0 (file deleted) | `include Buddy floater windows when a theme changes` | ast-grep rule `appearance-broadcast-reaches-all-windows` |
