...
2. **Wire the eval suite (M).** Promptfoo config + CI gate from the taxonomy (templates ready in the assistant workspace: `templates/evals/`), plus mechanical per-area pass/fail scoring for the review battery from its saved transcripts. This is the seatbelt for steps 4–8. Completing it triggers the `eval-runner` skill (assistant workspace ADR-004/ADR-012).
