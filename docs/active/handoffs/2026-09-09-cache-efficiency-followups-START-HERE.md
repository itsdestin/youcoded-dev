...
> **Status 2026-09-10 — MERGED (youcoded#464 `517e6253`, youcoded-dev#85).** Branch `session/cache-competitor-survey` in the `youcoded` repo (now deleted) (six commits, `verify.sh --full` green; an independent Opus code review on 2026-09-10 found no blockers and nine should-fix/nit items, all applied in the sixth commit — the notable one: the summary was reading a pruned copy of the span, which would have defeated its own warm prefix) lands items 1, 2, 3, 4, 6, 7 and the backend half of 8 as the survey's sequence table specifies: `providers/prompt-cache.ts` (Anthropic markers + OpenRouter `session_id`, keyed on `cacheKey`), `compaction.ts` `contextBudget` (reserve/trim/trigger from one function), the prune gate in `maybeCompact`, the warm-prefix summary request with its usage on `compact-summary`, `usage.expectedRebuild` on `turn-complete`, `cache-usage.ts` (llama.cpp `cache_n`, OpenRouter cache writes), and the Task tool byte-identity pin. Item 5 was dropped by the review (the summary shares the chat's key on purpose now). Depth: `youcoded/docs/native-runtime.md` → "Prompt caching". **Still open:** item 8's UI (a deck for Destin — four options in the survey), and the local measurement pass (`cache_n` is recorded per step now; nothing reads it yet). One visible change to approve: local compaction fires earlier (32k window: 21,196 tokens instead of 24,576) because the trigger now sits below the trim floor.
...
**Read next:** `docs/active/investigations/2026-09-09-cache-efficiency-competitor-survey.md` — how Hermes, pi, Unsloth Studio, OpenCode, Claude Code, Codex, Cline, Roo, Goose and Crush handle each of the eight items, with verified provider docs and a recommended "best version" and sequence per item. **Its sequence table supersedes the sizing below**: an independent context-free review on 2026-09-10 cut the plan to five small changes plus one design item, and corrected two things in this handoff: (a) item 3's prune is already batched by `planCompaction` (`'prune'` only when ≥ `minPruneSavings` is reclaimed); the per-step slide comes from `maybeCompact` also pruning on the `'summarize'` path when the summary then bails, so the fix is a gate, not a design; (b) item 4's store-side change is already shipped (`AcceptedHistoryTransformation` has `{ kind: 'summary' }`), leaving the summary call's cost and visibility. Cut with reasons: per-lane TTL (one `1h` for harness requests; summary gets a system-only marker because its `tool_choice` change would otherwise write the whole history at 2x for nothing); item 5's derived key (the warm-prefix summary should share the key; otherwise the pollution is marginal); item 7's roster-independent tool and snapshot (one miss per hire is the right price; pin the byte-identity with a test); sticky-boundary trimming and exact tokenization (unnecessary once compaction fires first, which is two constants); all engine flags (`--no-context-shift` and `--cache-ram` are already the defaults; `--cache-reuse` parked pending `cache_n` evidence); the cross-lane prefix-diff observer (an `expectedRebuild` flag on the step usage is enough to make regressions visible).

Authority and history: `docs/archive/investigations/2026-08-17-cache-efficiency.md` (the original four-provider study, provider cache rules, the "stable-prefix playbook"), `docs/archive/specs/2026-09-08-chatgpt-cache-efficiency-design.md` (what shipped in #461 and its privacy constraints), `docs/roadmap/native-harness.md` → "Cloud model context management and cache/token efficiency improvements" (the open roadmap item this handoff serves; its cache sub-bullet was the standalone "Cache efficiency" item until 2026-09-17). Destin's framing when he asked for this list: *"is there anywhere that users may be upset with us for wasting their money or otherwise breaking cache?"*
