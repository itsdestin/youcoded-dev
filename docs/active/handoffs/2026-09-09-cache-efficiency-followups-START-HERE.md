...
> **Status 2026-09-10 — built, awaiting Destin's merge decision.** Branch `session/cache-competitor-survey` in the `youcoded` repo (five commits, `verify.sh` green) lands items 1, 2, 3, 4, 6, 7 and the backend half of 8 as the survey's sequence table specifies: `providers/prompt-cache.ts` (Anthropic markers + OpenRouter `session_id`, keyed on `cacheKey`), `compaction.ts` `contextBudget` (reserve/trim/trigger from one function), the prune gate in `maybeCompact`, the warm-prefix summary request with its usage on `compact-summary`, `usage.expectedRebuild` on `turn-complete`, `cache-usage.ts` (llama.cpp `cache_n`, OpenRouter cache writes), and the Task tool byte-identity pin. Item 5 was dropped by the review (the summary shares the chat's key on purpose now). Depth: `youcoded/docs/native-runtime.md` → "Prompt caching". **Still open:** item 8's UI (a deck for Destin — four options in the survey), and the local measurement pass (`cache_n` is recorded per step now; nothing reads it yet). One visible change to approve: local compaction fires earlier (32k window: 21,196 tokens instead of 24,576) because the trigger now sits below the trim floor.
