...
| `docComments:reply` | request/response | `{path, id, text, author}` | real |
| `docComments:resolve` | request/response | `{path, id, by}` | real |
| `docComments:reopen` | request/response | `{path, id, by}` | real |
| `docComments:move` | request/response | `{path, id, newSelector}` — the re-anchor/repoint tool (§2, §5) | real |
...
`[sheet, cell]` pair (the full transcript, including any already-applied `​[[yc:resolved]]` marker —
§4.1 — since a move changes nothing about what was said or its resolve state, only where it points);
