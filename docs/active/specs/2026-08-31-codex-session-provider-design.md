...
status: draft
date: 2026-08-31
tags: [codex, openai, chatgpt, sessions, providers, permissions, native-runtime]
...
|---|---|---|
| Installed skills and marketplace plugins | Ours are written into Claude Code's four-file plugin registry. Codex reads none of it and has its own config (`~/.codex/config.toml`, its own MCP list). A Codex session is a bare agent. | A line on the session, not silence |
...
Six questions, no production code, about one day. Deliverable: a findings note, plus the
generator and its output **in the spike folder** — checked into `master` only after
Phase 1 signs off.
...
## 4. The five things with no existing answer
...
|---|---|---|---|
| **0** | The six questions in §3. Generator + output live in the spike folder only. | A findings note answers all six; the generator produces compiling types. | ~1 day |
| **1** | UI design and Destin's sign-off, built in `run-workbench.sh` against mock channels, reviewed as a deck via `scripts/ui-review/review-cards.py`. Covers: the network-approval card (§4.1) and its Settings row copy; how a Codex session is created and labelled (§9 Q1–2); model selection (§4.4); and every row of §1's "does NOT work" table — not-installed, not-signed-in, wrong-version, usage-limit, absent terminal tab, no skills. | Destin signs off on the deck. **Backend is designed around the approved UI, not the reverse.** | ~2–3 days |
...
Android. Making YouCoded skills, marketplace plugins or MCP servers work inside Codex
sessions (that waits on the `~/.youcoded` conventions inversion, ADR 008 —
`docs/decisions/008-conventions-inversion-native-home.md`). Migrating the Claude Code wrap
