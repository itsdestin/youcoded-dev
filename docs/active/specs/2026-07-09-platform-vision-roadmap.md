...
8. **Subagents design handoff (bridge to Phase 4):** Task/subagents were deferred from Phase 2 by explicit decision but are core/vital — child sessions with a parent-session pointer and a condensed result back are the mechanism Phase 4's headless runner and agent model both assume exists. Phase 2's loop + session store were designed so this lands without a schema change; scope the design here even if implementation lands with Phase 4 (ROADMAP Features, added 2026-07-15).
