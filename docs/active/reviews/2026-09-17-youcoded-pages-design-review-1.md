...
triage: accepted — the skill uses the transport's own repo (`--git-dir=<root>/.youcoded/sync.git --work-tree=<root>`); project pages get history only where the project is a sync space or a git repo, and the skill says so.
...
triage: accepted — data.json gets a `{ savedAt, data }` envelope and folds by savedAt under lock; page.html keeps the sync policy (remote wins, our copy deleted under lock) with a matcher for .html copies.
...
triage: accepted — ids are `personal:<slug>` and `project:<project name>:<slug>`; the renderer already has the path from PageHome.
...
triage: accepted — `get()` returns the data and PageHost bakes `window.youcoded.data` into the document like the theme; the listener checks `e.source === frame.contentWindow`; the 1 MB cap is enforced in main as well.
...
triage: accepted — `initialInput` is added to SessionCreateRequest/SessionCreateArgs and the Kotlin session:create arm; the Claude Code lane depends on the plugin being installed, which the bundled reconcile does on the built app.
...
triage: accepted — task order changed: the plugin is written first and installed by hand for the dev walk-through.
...
triage: accepted — summaries carry an `htmlStamp` (page.html mtime); PageHost refetches on `[pageId, htmlStamp]`; data writes never touch the stamp.
...
triage: accepted — no watcher per project; the store subscribes to project-watcher events for paths under `Pages/`; a missing folder is re-checked on every `list()`.
...
triage: accepted — `Pages` joins the discovery skip set for Phase 1; revisit when pages can be attached to a conversation.
...
triage: accepted — the walk-through uses a throwaway page and deletes it after; all pin and data writes go through mutateFileUnderLock.
...
triage: accepted — `updatedAt` and `htmlStamp` come from mtime; the store rejects a slug that collides case-insensitively.
...
triage: accepted — citations corrected in the design.
