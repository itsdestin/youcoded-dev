...
- F7 deferred (filed) — the comment claimed a UI that does not exist. Comment corrected; the unknown-outcome indicator is filed to the roadmap rather than invented here.
...
- F10 rejected as a defect, recorded as scope. R6 is `checkedBy: deck` and browser encryption is the OPTIONAL second level, deliberately mockup-only in this batch. Called out to Destin so nothing reads it as shipped.
...
- F16 deferred (needs the second device) — R3 is `checkedBy: live-app` and needs the second device this batch has never had. Recorded as such for the grader rather than claimed.
