...
- deleted/merged: merged the duplicated UI-sweep bullet in `react-renderer.md` into its `/ui-review` pointer — but the rule still went 666 → 759 words against the ≤600 convention it was ALREADY over; the added invariants are measured and load-bearing, and trimming other sessions' entries was not mine to do
