...
1. **SyncHub is NOT in this plan.** It's blocked on accounts Phase 1 (spec §17 sequencing note). The engine ships with the spec §6 degradation path — poll the transport every 2 minutes — as its initial signal mechanism. Plan 1b adds SyncHub and drops the poll interval to a fallback.
