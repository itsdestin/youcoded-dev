...
4. **Fix-on-sight.** A doc known to be wrong gets fixed the moment the inconsistency is verified — there is no drift ledger. Deferred-fix bookkeeping cost more than the fixes. The one escape valve: drift that genuinely can't be fixed in the current session (needs Destin's decision, an Android device, or would derail critical in-flight work) becomes a ROADMAP.md `bug` line tagged `#docs` — an existing store, not a resurrected ledger.
...
| Doc found contradicting code | **Fixed immediately** (verify against code first, cite what was verified in the commit) | knowledge-debt.md is deleted; no ledger. Unfixable-in-session drift → ROADMAP `bug` line tagged `#docs` |
...
| Destin-specific preferences, working-style feedback, cross-session context with no home above | Auto-memory (`~/.claude/projects/.../memory/`) | **Last resort** — if a row above fits, it goes there, never memory. Product planning never lives in memory (roadmap knowledge currently parked there migrates out in Phase 1) |
...
**Triage discipline:** the implementer executes high-confidence dispositions directly, recording each entry's disposition (entry → delete / one-liner / rule / sub-repo / keep, with destination) in a changelog committed alongside the change — an "I did the following" confirmation, not a pre-approval gate. Only low/medium-confidence entries are queued for Destin's review, each presented with a recommendation. **Eager-tier criterion:** an entry where a *single* violation is catastrophic (the live-app-safety class — irreversible damage to Destin's working environment or data) stays eager (CLAUDE.md or a `paths: "**"` rule); entries where violation ships a catchable bug can be rule- or doc-scoped.
...
- **Workspace gardening (meta-pass):** enforce the store budgets from principle 5 (eager load ≤ ~10k tokens; per-rule ~600 words; slim PITFALLS ~2,500 words) and trim/migrate regressions; detect subsystems that gained code but lack a path-scoped rule; verify `docs/MAP.md` anchors and update the map for new/renamed entry points; sweep `docs/active/` for docs whose feature shipped and move them to `docs/archive/`; verify status frontmatter presence; dedup ROADMAP items; review auto-memory for duplicative/misplaced/drifted entries (planning content migrates to ROADMAP.md — memory is the last-resort store per the taxonomy); review outward-facing docs (README/privacy/license/sub-repo CLAUDE.md) against the diff since last audit. This is the enforcement mechanism that keeps the Phase 1–2 cleanup from re-rotting.
...
- Everything in `docs/active/` carries `status:` frontmatter: `draft | active | shipped | superseded` (+ `shipped:` commit/PR where known — `shipped` covers both merged plans and implemented specs), so sweeps stay mechanical. Handoff docs also carry an explicit expiry condition ("archive when X merges").
...
- Add the **archive grep-hygiene rule**: searches for live docs exclude `docs/archive/` by default (the convention only exists if it's written here).
...
The hook (`.claude/hooks/context-inject.sh`) has two halves. The **project-state injection** (per-repo branch / recent commits / dirty files, active worktrees) is untouched — it's dynamic state no doc or rule can carry, and it's what keeps sessions from working against stale branches. Only the **staleness tail** changes:

- Drop the knowledge-debt nag (file is deleted).
- Staleness signal points at the latest `docs/audits/` report: warn if >60 days old or if its `residue:` frontmatter count is non-zero (unapplied findings).
...
1. **Phase 1 — mechanical wins + census:** de-`@import` CLAUDE.md; create ROADMAP.md (seeded from knowledge-debt triage + known deferred work + `idea` entries for the Project View Roadmap tab and rolling roadmap cleanup-by-release); delete knowledge-debt.md + GEMINI.md; the document census — sweep workspace + all sub-repos for lifecycle docs and sort into `docs/active/`/`docs/archive/` with status + `origin:` frontmatter (dissolving `docs/superpowers/`); baseline review of READMEs, privacy copy, licenses, and sub-repo CLAUDE.md files; one-time auto-memory review (duplicative/misplaced/drifted entries; planning content → ROADMAP.md); hook update.
2. **Phase 2 — PITFALLS triage** (judgment-heavy; implementer executes high-confidence dispositions with a committed changelog, Destin reviews only the low/medium-confidence residue): the 5-way triage above, including the existing `docs/*.md` subsystem docs; expand `.claude/rules/` (with `verify:` anchor blocks); create `docs/MAP.md`; retire the `context-*` skills; add pinning tests where high-value invariants are unguarded.
...
Phases 1 and 2 ship in direct succession (same working session or back-to-back), so the window where PITFALLS content is neither eager-loaded nor rule-scoped is deliberately not bridged.
