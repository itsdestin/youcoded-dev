...
**Date:** 2026-07-03
**Status:** Approved design — **Phase 2 MERGED 2026-07-09** (worker: wecoded-marketplace `f85403d` PR#20, deployed + smoke-tested — social/ module, PresenceRoom DO, /auth/export, handle self-reclaim during cooldown via `handle_releases.released_by`; client: youcoded `814365c4` — friends UI, platform-owned presence socket, blocked-users + Download my data in Settings, account-context rename, all 8 Phase 1 knowledge-debt items, privacy copy approved). Phase 2 extras beyond the plans: handle self-reclaim (Destin request), room-code UI removed entirely — challenges are the only game entry (Destin decision 2026-07-09; codes remain the internal PartyKit capability token). **Two-person verification is DEFERRED to post-release** (Destin: needs a second person on a released build) — the spec §7 checklist (add-by-handle → accept → mutual presence → challenge → full game → block → last-seen) should run in the wild after the next release. Phase 1 MERGED 2026-07-08 (worker `8d18246` PR#18; client `3d62baa4`, runtime-verified). Phase 0 MERGED (youcoded `10eac9ca`). Phase 3 (inboxes, Google provider, tier-C moderation) sketched — needs its own design pass.
**Supersedes:**
