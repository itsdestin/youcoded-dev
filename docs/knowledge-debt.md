...
## Sync spaces: no import path for existing folders (noticed 2026-07-09)
- **Claim**: Spec §3 pitches "Turn on Sync and your projects are on every device," and §3's import bullet says existing external folders can be imported into `Projects/`.
- **Actual**: Phase 1a (youcoded#107) shipped sync only for folders created inside `~/YouCoded/` via the new-project form. There is NO flow to convert an existing saved-folder project (e.g. `~/askthebudgetaz`, `~/cookinonlowheat`) into a synced project, and no folder-picker import. For a machine full of pre-existing projects, sync currently covers none of the user's real work.
- **Fix**: Write + execute a small plan for the two §3 import flows (spec updated 2026-07-09 with full requirements; §17 "Phase 1-followup" owns it): (1) "Sync this project" on existing picker folders — plain-language move-with-consent warning, `fs` move into `~/YouCoded/Projects/<name>/`, update `~/.claude/youcoded-folders.json`, remap artifact sidecar/central-index/conversation cwd references, guard against live sessions on the folder; (2) folder-picker import in the new-project flow with the same treatment. Move (not copy) is the default. Verify with a live dev-window test moving a scratch folder.
- **Priority**: high (spec-declared completion prerequisite for the sync rebuild; blocks calling Phase 1 done)
