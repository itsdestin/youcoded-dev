...
- **Fix**: v1.3.1 dedicated pass. (a) Bug fix: `engine-supervisor.ts start()` — `fs.mkdirSync(cacheDir,{recursive:true})` before `spawn()`, and add a `child.stderr` listener that buffers the tail and includes it in the thrown error. (b) Workspace-wide: audit every user-facing `throw new Error`, toast, banner, and IPC error string (desktop + Android + worker) against `docs/error-message-standards.md` — make each either specific+accurate (thread real subprocess/exception detail through) or general+non-committal with the two-action fallback (Report bug / Diagnose with Claude). (c) Build the two-action fallback as a reusable component.
- **Priority**: medium (correctness/UX; the engine `mkdirSync` + stderr-capture sub-fix is small and high-value — every first-time local-models user hits the misleading engine error).
