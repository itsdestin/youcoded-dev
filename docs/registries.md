...
- `marketplace.json` — DestinCode-only entries
- Synced from upstream via `scripts/sync.js`. Entries with `sourceMarketplace: "destinclaude"` are never overwritten by upstream sync
