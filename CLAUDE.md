...
**A failing or flaky test is fixed when it is found, never filed as a roadmap item** — even if it predates your change or looks unrelated (Destin, 2026-09-17). In read-only work, report it and ask. If it truly cannot be fixed here (e.g. a Windows-only failure), tell Destin why rather than filing it. Diagnosis steps: `.claude/rules/test-suite-hygiene.md` → Before calling a failure "flake".
...
When the SDK IS present, `ANDROID_HOME` is still unset, so name both: `JAVA_HOME=<a JDK 21> ANDROID_HOME=$HOME/.android-sdk ./gradlew test -x bundleWebUi` from `youcoded/` (`-x bundleWebUi` protects hardlinked dependencies). Find a JDK with `ls -d /usr/lib/jvm/* /opt/*/jbr 2>/dev/null` — the path that works has changed repeatedly, so name no favourite: on 2026-09-11 that list came back EMPTY while the SDK itself was present, and the system Java (26) fails Gradle's `jlink` step for `android-35`, which reads as a build error rather than a missing JDK. Read the count out of `app/build/test-results/` — a green BUILD SUCCESSFUL with everything up-to-date is not a test result. When it is ABSENT, Gradle stops at `SDK location not found` during configuration: say the Android half is unverified rather than implying a run happened, and do not edit this paragraph to assert absence — that is the loop.
