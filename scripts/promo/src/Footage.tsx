...
type Props = { file: Scene; from?: number; rate?: number; pushIn?: number; scale?: number; dy?: number; opacity?: number; light?: boolean };
// One clip inside the window, trimmed by frame and optionally sped up (`rate`)
// so a long recording fits its bars.
export const Footage: React.FC<Props> = ({ file, from = 0, rate = 1, pushIn = 0, scale, dy, opacity, light }) => (
  <Window pushIn={pushIn} scale={scale} dy={dy} opacity={opacity} light={light}>
