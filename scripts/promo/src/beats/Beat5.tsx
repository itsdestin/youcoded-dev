...
// the sorted sheet is a static screen: at 0.74× the clip's last 96 frames fill its two bars
const B_RATE = 0.74;
assertClipCovers('promo-sheet', B_FROM, T_PROJ - T_AFTER, B_RATE);
...
const Beat5: React.FC = () => (
  <AbsoluteFill>
    ...
    <Sequence from={T_AFTER} durationInFrames={T_PROJ - T_AFTER}><Footage file="promo-sheet" from={B_FROM} rate={B_RATE} pushIn={0.02} light /></Sequence>
    <Sequence from={T_PROJ}><Footage file="promo-project" from={P_FROM} rate={P_RATE} light /></Sequence>
    <Sequence durationInFrames={T_PROJ}><Label text={CAPTIONS.b5.head} at={B('b5', 0) + 4} slug="meadow-mist" /></Sequence>
