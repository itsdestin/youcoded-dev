#!/usr/bin/env bash
# Films every promo scene against <worktree>, into scripts/promo/footage/, and
# writes the footage-review page for Destin.
...
# This used to boot its own workbench on a fixed port (:5473, offset 300) and
# refuse to run against a foreign one serving the wrong tree — record.mjs now
# builds+serves $TDIR itself through scripts/shoot/engine.mjs (WORKTREE=$TDIR
# below), on a free port, cached after the first scene, so there is nothing
# left here to boot or guard.
...
for s in "${SCENES[@]}"; do
  echo "[film] $s"
  if WORKTREE="$TDIR" node "$WS/scripts/ui-review/record.mjs" "$WS/scripts/ui-review/scenes/$s.json" "$OUT/$s"; then
