...
F=$(node -e 'const c=require(require("path").resolve(process.argv[1]));console.log(c.map(x=>x.at+14).join(","))' "$OUT/cues.json")   # resolve(): a relative outdir made require() look for a package (2026-09-09)
