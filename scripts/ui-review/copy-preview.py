...
ROWS = [
    ('row1', 'One app, any AI', 'Pick the AI. Keep the conversation.',
     'Claude with your plan. Hundreds of models through OpenRouter. Or a model that runs on your own computer — free, offline, private. Switch mid-conversation; nothing is lost.',
     'LOOP · Midnight — model picker opens, switches Claude → an OpenRouter model → a local model; the chat continues; the status-bar model chip changes.', '', '', ''),
    ('row2', 'It actually does things', 'Give it a folder. It asks before it touches anything.',
     'It reads your files, writes new ones, runs the steps, and searches the web. Every action that changes something shows up as a question first — and every "always allow" you grant is listed in one screen where you can take it back.',
     'LOOP · Crème — "turn the receipts in my Receipts folder into a spreadsheet" typed; Read cards; a permission ask answered; then the Permissions settings screen listing the remembered rule.',
    ...
    ('row3', 'Your projects, in one place', 'Files, chats, and instructions — together.',
     'Open spreadsheets, PDFs, documents, and images without leaving the app. Edit a file next to the conversation about it. Search everything you\'ve ever said in a project.',
     'LOOP · Light — Project View: Files tab opens a PDF and a spreadsheet in-app; the editor pane; one project-wide search.', '', '', ''),
    ('row4', 'Stay organized', 'Tags, notes, and one-tap shortcuts.',
     'Tag and annotate conversations, filter by tag, pin the ones that matter. Quick chips run the things you do every day in one tap. A project\'s instructions are a tab you can read and edit, not a hidden file.',
    ...
    ('row5', 'Follow you everywhere', 'Start on your laptop. Finish on your phone.',
    ...
    ('row8', 'For builders', 'Claude Code inside, your own agent beside it.',
