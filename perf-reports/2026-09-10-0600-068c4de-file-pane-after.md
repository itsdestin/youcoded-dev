# perf-lab 2026-09-10-0600-068c4de-file-pane-after

sha 068c4de77f6949e1476f02110757d54c90cae607 (fix/file-pane-spawns-and-redraw) — 2026-09-10T06:00:39.438Z
...
| idle PSS | — |
| idle CPU | — |
| switch, pane swapped (median of 3) | 76.3 ms / 233.4 ms p95 |
| **switch, messages on screen** | **100.5 ms / 142.7 ms p95** |
| switch into a 'huge' conversation (n=3, 60 entries of 60 expected) | 118.9 ms / 119.1 ms p95 |
| switch into a 'medium' conversation (n=3, 152 entries of 152 expected) | 94.8 ms / 114.8 ms p95 |
| switch into a 'small' conversation (n=3, 166 entries of 166 expected) | 89.6 ms / 130.6 ms p95 |
| switch into a 'empty' conversation (n=3, 0 entries of 0 expected) | 101.3 ms / 117.4 ms p95 |
| switch into a 'native' conversation (n=6, 1 entries) | 100.5 ms / 122.7 ms p95 |
| streamed into, during the switches | medium, small |
| long tasks | 41 tasks (4163 ms total, max 326 ms) |
| frame gaps > 40ms | 63 gaps (max 343 ms) |
| native first token | 1081 ms |
| **layouts per streamed token (native)** | **stream-too-slow** — 35 layouts over 35 commits / 181 frames (0.193 /frame, 1 /commit) — ⚠ NOT a clean reading |
| CPU during workload | 650.7 % |
| PSS after workload | 1687.1 MB |
| artifacts.open code small / large (median of 3) | 54 ms / 89 ms |
| artifacts.open markdown small / large | 861 ms / 1459 ms |
| artifacts.html swap median / p95 | 129 ms / 175 ms |
| artifacts.keystroke small median / p95 | 26.2 ms / 32.5 ms |
| artifacts.keystroke large median / p95 | 25.6 ms / 31.1 ms |
| artifacts.copy click -> "Copied!" | 23.9 ms |
| artifacts long tasks | 2424 ms total, max 739 ms |
| artifacts IPC stall (sum over steps) | 286 ms, max 323 ms, from 112 probe replies |

noise: load 1.8, busy 4.1%, worst accepted load 3.42 / busy 4.4%, discarded 4
