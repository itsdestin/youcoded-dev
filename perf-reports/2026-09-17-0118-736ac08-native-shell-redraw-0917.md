# perf-lab 2026-09-17-0118-736ac08-native-shell-redraw-0917

sha 736ac08eef6184724fb044e66b50ea087a37afe8 (perf/shell-redraw-per-token) — 2026-09-17T01:18:26.939Z
...
| idle PSS | — |
| idle CPU | — |
| **native-stream.visible: renderer main thread busy while a 3000 delta reply streams on screen at 150 /s** (median of 3; achieved 150 /s, 9780 chars shown) | **8859.2 ms (44.2 % of the window)**, script 6376 ms; long tasks 0 ms total / max 0 ms, worst frame gap 0 ms, 60 fps; verdict none |
| native-stream.visible commits / layouts over the stream | 1203 commits (1 /frame), 1208 layouts (1.004 /frame); turn 20057 ms |
| native-stream.visible IPC stall | 0 ms, max 5 ms, from 200 probe replies |
| **native-stream.switching: click -> messages on screen while it streams** (8 verified of 8 during the stream) | **72.4 ms / 78.5 ms p95**; into huge 75 ms, into the streaming one 29.2 ms; main thread busy 8355.3 ms (41.7 %), long tasks 0 ms |
| **native-stream.hidden: the visible window's main thread while the reply streams into a hidden session** | **1357 ms busy (6.3 %)**, long tasks 0 ms / max 0 ms, 0 commits in the visible pane, IPC stall 0 ms |
| **native-resume.browse: Resume list open over 100 native sessions** (median of 3) | **189 ms** to 50 rows; revealed 703 rows in 3396 ms; IPC max 12 ms; verdict none |
| **native-resume.resume: 400-turn native session resumed** | **280 ms** (pill 229 ms, 60 entries); IPC max 65 ms, stall 15 ms; verdict none |
| native-resume.page-up one history page | 136 ms (+60 entries); IPC max 5 ms |
| native-resume.turn 300 deltas | 2046 ms; IPC max 1 ms, stall 0 ms; verdict none |
| native-resume.tearoff into a new window | 24 ms (2 windows); IPC max 1 ms, stall 0 ms; verdict none |
| native-resume.turn into the detached window, this window's IPC | max 7 ms, stall 0 ms |
| **native-resume IPC stall (sum over steps)** | **15 ms**, max 65 ms, from 252 probe replies; 0 steps lost their reading |

noise: load 1.58, busy 2.4%, worst accepted load 2.82 / busy 4.4%, discarded 3
