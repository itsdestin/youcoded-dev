# perf-lab 2026-09-16-2352-2a63c54-click-paths-0916

sha 2a63c54e57b381f40cfaba87691d4f31477ac5a1 (perf/main-thread-click-paths) — 2026-09-16T23:52:01.717Z
...
| idle PSS | — |
| idle CPU | — |
| switch, pane swapped (median of 3) | 45.5 ms / 112.3 ms p95 |
| **switch, messages on screen** | **73.4 ms / 106.3 ms p95** |
| switch into a 'huge' conversation (n=3, 60 entries of 60 expected) | 89.4 ms / 90.5 ms p95 |
| switch into a 'medium' conversation (n=3, 152 entries of 152 expected) | 55.8 ms / 61.6 ms p95 |
| switch into a 'small' conversation (n=3, 166 entries of 166 expected) | 47.4 ms / 70 ms p95 |
| switch into a 'empty' conversation (n=3, 0 entries of 0 expected) | 72.9 ms / 90.5 ms p95 |
| switch into a 'native' conversation (n=6, 1 entries) | 73.8 ms / 89.5 ms p95 |
| streamed into, during the switches | medium, small |
| long tasks | 15 tasks (1404 ms total, max 189 ms) |
| frame gaps > 40ms | 44 gaps (max 194 ms) |
| native first token | 1138 ms |
| **layouts per streamed token (native)** | **stream-too-slow** — 40 layouts over 41 commits / 180 frames (0.222 /frame, 0.95 /commit) — ⚠ NOT a clean reading |
| CPU during workload | 301.9 % |
| PSS after workload | 1676.2 MB |

noise: load 2.35, busy 6.7%, worst accepted load 3.72 / busy 6.7%, discarded 1
