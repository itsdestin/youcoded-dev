# perf-lab 2026-09-24-0110-a989718-wl3-before

sha a9897183fda005d45d0b1e868c803b6be2ef1066 (session/perf-many-tabs-base2) — 2026-09-24T01:10:26.981Z
...
| idle PSS | — |
| idle CPU | — |
| switch, pane swapped (median of 5) | 38.8 ms / 157.9 ms p95 |
| **switch, messages on screen** | **89.1 ms / 175.4 ms p95** |
| switch into a 'huge' conversation (n=3, 60 entries of 60 expected) | 86.9 ms / 173.8 ms p95 |
| switch into a 'medium' conversation (n=3, 152 entries of 152 expected) | 104.7 ms / 112.7 ms p95 |
| switch into a 'small' conversation (n=3, 166 entries of 166 expected) | 115.5 ms / 135 ms p95 |
| switch into a 'empty' conversation (n=3, 0 entries of 0 expected) | 76.4 ms / 102.4 ms p95 |
| switch into a 'native' conversation (n=6, 1 entries) | 72.8 ms / 91.2 ms p95 |
| streamed into, during the switches | medium, small |
| long tasks | 24 tasks (2108 ms total, max 188 ms) |
| frame gaps > 40ms | 42 gaps (max 194 ms) |
| native first token | 2300 ms |
| **layouts per streamed token (native)** | **stream-too-slow** — 41 layouts over 42 commits / 181 frames (0.227 /frame, 0.974 /commit) — ⚠ NOT a clean reading |
| CPU during workload | 272.4 % |
| PSS after workload | 1720.3 MB |

noise: load 2.61, busy 5.1%, worst accepted load 3.63 / busy 8.1%, discarded 1
errors (desktop.log "level":"ERROR" lines): cold starts [], scenario boot —, workload boots [0,0,0,0,0], stall boot —, artifacts boot —, projects boot —, terminal boots [], native-stream boots [], native-resume boots [], scrollback boot —
