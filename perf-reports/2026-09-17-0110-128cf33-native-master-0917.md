# perf-lab 2026-09-17-0110-128cf33-native-master-0917

sha 128cf3343e83ddf626f4cc7c840eb784576a5a78 (session/perf-baseline-20260916) — 2026-09-17T01:10:00.834Z
...
| idle PSS | — |
| idle CPU | — |
| **native-stream.visible: renderer main thread busy while a 3000 delta reply streams on screen at 150 /s** (median of 3; achieved 150 /s, 9780 chars shown) | **11813.1 ms (58.9 % of the window)**, script 9307.5 ms; long tasks 0 ms total / max 0 ms, worst frame gap 0 ms, 60 fps; verdict none |
| native-stream.visible commits / layouts over the stream | 1202 commits (1 /frame), 1208 layouts (1.004 /frame); turn 20057 ms |
| native-stream.visible IPC stall | 0 ms, max 16 ms, from 200 probe replies |
| **native-stream.switching: click -> messages on screen while it streams** (8 verified of 8 during the stream) | **72.9 ms / 76.8 ms p95**; into huge 75.4 ms, into the streaming one 31.2 ms; main thread busy 9001.3 ms (44.9 %), long tasks 0 ms |
| **native-stream.hidden: the visible window's main thread while the reply streams into a hidden session** | **1049.4 ms busy (4.9 %)**, long tasks 0 ms / max 0 ms, 0 commits in the visible pane, IPC stall 0 ms |
| **native-resume.browse: Resume list open over 100 native sessions** (median of 3) | **193 ms** to 50 rows; revealed 703 rows in 3397 ms; IPC max 22 ms; verdict none |
| **native-resume.resume: 400-turn native session resumed** | **281 ms** (pill 232 ms, 60 entries); IPC max 174 ms, stall 124 ms; verdict none |
| native-resume.page-up one history page | 134 ms (+60 entries); IPC max 4 ms |
| native-resume.turn 300 deltas | 2047 ms; IPC max 1 ms, stall 0 ms; verdict none |
| native-resume.tearoff into a new window | 23 ms (2 windows); IPC max 1 ms, stall 0 ms; verdict none |
| native-resume.turn into the detached window, this window's IPC | max 3 ms, stall 0 ms |
| **native-resume IPC stall (sum over steps)** | **130 ms**, max 174 ms, from 252 probe replies; 0 steps lost their reading |

noise: load 1.09, busy 2.1%, worst accepted load 2.71 / busy 6.5%, discarded 1
